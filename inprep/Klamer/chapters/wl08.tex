...
\langinfo{}{}{}\\
\glll {\rm Kamang\ilt{Kamang}}  lami  ge-  kadii \\  
     {\rm Abui\ilt{Abui}}     neng  he-  fala  \\
...
\langinfo{}{}{}\\
\glll {\rm Kamang\ilt{Kamang}}   lami  ge- su \\  
    {\rm Abui\ilt{Abui}}     neng  he-  sua   \\
...
Kamang\il{Kamang} has an irregular choice of reduplicative\is{reduplication} base. Compare the reduplicants\is{reduplication} (in bold-face) in the numerals `seven' to `nine' in \REF{ex:8:1236}. We see that Abui\il{Abui} reduplicates\is{reduplication} only the right-most numeral, resulting in word-internal reduplication\is{reduplication}, which is consistent with the regular distributive\is{distributive numerals} pattern in AP languages ({\S} \ref{sec:8:Regulardistributive}). By contrast, Kamang\il{Kamang} reduplicates\is{reduplication} the initial element \textit{wesing}. As main stress is on the final syllable of the numerals in Kamang\il{Kamang} just as it is in Abui\il{Abui}, we analyse this as a choice of reduplicative\is{reduplication} base in Kamang\il{Kamang} distributives\is{distributive numerals} which diverges from the overall pattern of AP languages. 
...
Kamang\il{Kamang} expresses multiplication with an applicative verb derived from a cardinal\is{cardinal numerals} base by prefixing \textit{mi-}. (Compare Teiwa\il{Teiwa}, where the applicative derivation is used for fractions, see {\S} \ref{sec:8:3.5}). 
...
Note that the order of the verb relative to its complement `two' in \REF{bkm:Ref342664493}-\REF{bkm:Ref342746525} is unexpected, as it goes against the canonical AP object-verb order, found in subtraction ({\S} \ref{sec:8:3.2}). Note that the equivalent expression in Indonesian\il{Indonesian}/Malay\il{Malay} is \textit{sepuluh bagi dua (adalah)} \textit{lima} lit. `ten divide two (is) five', with verb complement order. It may be the case that the constructions in \REF{bkm:Ref342664493}-\REF{bkm:Ref342746525} are calques from Indonesian\il{Indonesian}/Malay\il{Malay}. 
