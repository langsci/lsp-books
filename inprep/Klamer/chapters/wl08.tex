...
\begin{abstract}
The indigenous numerals of the AP languages, as well as the indigenous structures for arithmetic operations are currently under pressure from Indonesian\il{Indonesian}, and will inevitably be replaced with Indonesian\il{Indonesian} forms and structures. This chapter presents a documentary record of the forms and patterns currently in use to express numerals and arithmetic operations in the Alor-Pantar languages. We describe the structure of cardinal\is{cardinal numerals}, ordinal\is{ordinal numerals} and distributive numerals\is{distributive numerals}, and how operations of addition, subtraction, multiplication, division, and fractions are expressed.
\end{abstract}

\todo{you can use {\textbackslash}todo\{Doe iets\} to leave comments in the text}
...
The patterns described in this chapter fall into two broad classes, pertaining to two distinct linguistic levels: the word (Section \ref{sec:8:Operations}) and the clause or sentence (Section \ref{sec:8:Structures}). At the word level we describe how numeral words are created, discussing the structure of cardinals\is{cardinal numerals} (Section \ref{sec:8:Cardinal}), ordinals\is{ordinal numerals} (Section \ref{sec:8:Ordinal}) and distributives\ist{distributive numerals} (Section \ref{sec:8:Distributive}). At the clause and sentence level, we describe the constructions that contain numerals and function to express the arithmetic of addition (\ref{sec:8:Addition}), subtraction (Section \ref{sec:8:Subtraction}), multiplication (Section \ref{sec:8:Multiplication}), division (Section \ref{sec:8:Division}), and fractions (Section \ref{sec:8:Fractions}). Section \ref{sec:8:Summary} presents a summary and conclusions. 

Details on the data on which this chapter is based is given in the  Sources section at the end of this chapter. Adang\il{Adang}, Blagar\il{Blagar}, Kamang\il{Kamang} and Abui\il{Abui} are each very diverse internally. The data presented in this chapter are from the Dolap dialect of Blagar\il{Blagar}, the Takailubui dialect of Kamang\il{Kamang}, the Lawahing dialect of Adang\il{Adang}, and the Takalelang dialect of Abui\il{Abui}. These dialect names refer to the place where the variety is spoken.


\section{Operations to create numeral words} 
\label{sec:8:Operations}
\todo{You can doubleclick in the pdf to go to the corresponding passage in the text}
Most of the cardinals\is{cardinal numerals} in AP languages are historically morphologically complex forms. Within and across the languages we find variation in choice of numeral base, the type of operations invoked for the interpretation of the composite elements, and the ways in which these operations are expressed (Section \ref{sec:8:Cardinal}). Ordinals\is{ordinal numerals} in AP languages are possessive\is{possession} constructions that are derived from cardinals\is{cardinal numerals}, where the ordered entity is the grammatical possessor\is{possession} of the cardinal\is{cardinal numerals} (Section \ref{sec:8:Ordinal}). Distributive numerals\is{distributive numerals} in AP languages are also derived from the cardinal\is{cardinal numerals}, by reduplicating\is{reduplication} it partially or fully. When the cardinal\is{cardinal numerals} contains more than one morpheme, generally only the right-most formative is reduplicated\is{reduplication} (section \ref{sec:8:Irregularities}).  In all cases, the numeral words follow the noun they quantify. Cardinals\is{cardinal numerals} may be preceded by a classifier\is{numeral classifier}, if the language has them. 

\subsection{Cardinal numerals}
\label{sec:8:Cardinal}
By cardinal\is{cardinal numerals} numerals, we understand the set of numerals used in attributive quantification of nouns (e.g., `three dogs'). In enumeration, the numeral follows the noun in all AP languages (N NUM), as in Teiwa\il{Teiwa} \textit{yaf haraq} `house two' {\textgreater} `two houses'. If a language uses a sortal or mensural classifier\is{numeral classifier}, the classifier\is{numeral classifier} occurs between the noun and the numeral (N CLF NUM). The same cardinals\is{cardinal numerals} that are used in enumeration are also used for non-referential counting (\textit{one, two, three, four, five, etc.}), and all the AP languages use the same numeral forms to count small animates (ants, flies, bees, or house lizards), large animates\is{animacy} (children, dogs, or pigs), and inanimates\is{animacy} (houses, rocks, stars, or coconut trees).

In all the AP languages we surveyed, the cardinal\is{cardinal numerals} numbers `one' to `five' are morphologically simple forms, as illustrated in Table \ref{tab:8:1}. The composition of `six' varies. Most of the AP languages have a monomorphemic `six', an example is Teiwa\il{Teiwa} \textit{tiaam}. Bi-morphemic forms for `six' are composed of (reflexes) of `five' and `one', e.g., Kula\il{Kula} \textit{yawaten sona}. The cardinals\is{cardinal numerals} `seven' and higher consist of minimally two formatives in all AP languages. Often, these forms involve reflexes of `five', `one', `two', `three' and `four', as illustrated in Table \ref{tab:8:1}, though other patterns are also attested \citep{SchapperKlamerTV}. 
...
Third, different types of operations are involved in the derivation of cardinals\is{cardinal numerals}: typically they involve simple juxtaposition of bases (e.g., Abui\il{Abui} \textit{kar nuku} `ten' {\textless} \textit{kar} `ten', \textit{nuku} `one'), but in some cases, a lexeme is added that expresses the operation (e.g., the operator \textit{wal} signifying addition in numerals 11-19, e.g., Abui\il{Abui} \textit{kar nuku} \textbf{\textit{wal}}\textit{nuku} `eleven'. 
...
\todo[inline]{fix broken crossref by inserting corresponding label bkm:Ref355363568 between {\textbackslash}REF\{\}}\footnote{{}  Compare proto-Alor-Pantar\ilt{proto-Alor-Pantar} *hasak \citep{HoltonRobinsonTV}, reflected in Western Pantar\ilt{Western Pantar} \textit{hakkas}, Kaera\il{Kaera} \textit{isik}, Abui\ilt{Abui} \textit{taka}, Kamang\ilt{Kamang} \textit{saka}.} In the Teiwa\ilt{Teiwa} idioms in \REF{bkm:Ref355363568}, a subject precedes a nominal predicate that is headed by the place pronoun\is{pronoun} \textit{i} `it.(place)', so that absent entities are expressed as ``X is (an) empty place(s)'', compare (\REF{bkm:Ref355363568} (a-b).
...
In sum, AP languages have up to six morphologically simple cardinals\is{cardinal numerals}; in all AP languages, the non-borrowed\is{borrowing} cardinals\is{cardinal numerals} `seven' and up are morphologically complex. Most cardinals\is{cardinal numerals} are compounds, consisting of two or more morphemes in apposition, the second of which gets word stress. The definition of the morphological structure of these compounds varies along three dimensions: the choice of base, the arithmetic operations invoked for the interpretation of the cardinals\is{cardinal numerals} that make up the numeral, and the ways in which these arithmetic operations are expressed. 

\subsection{Ordinal  numerals\ist{ordinal numerals}}
\label{sec:8:Ordinal}
Ordinal\is{ordinal numerals} numerals are words that identify the position that a given member of a set occupies relative to other members of the same set (e.g., `the third dog'). The main function of ordinal\is{ordinal numerals} numerals is thus to indicate the position of an entity in an ordered sequence. 

All AP languages have distinct forms for cardinal\is{cardinal numerals} and ordinal\is{ordinal numerals} numerals, and all of them have ordinal\is{ordinal numerals} numbers associated with any cardinal\is{cardinal numerals} from `two' and above. Ordinals\is{ordinal numerals} in AP languages are derived from cardinals\is{cardinal numerals}, which is a cross-linguistically common pattern \citep{StolzEtAl2013}. Variation exists only in the expression of `first', which in some of the languages is unrelated to the numeral `one', as discussed below. 

The derivation of ordinals\is{ordinal numerals} involves a third person possessive\is{possession} pronoun\is{pronoun} or prefix at the left periphery of the cardinal\is{cardinal numerals} numeral. The ordered entity functions grammatically as the possessor\is{possession} of the cardinal\is{cardinal numerals} number. For example, Kamang\il{Kamang} \textit{dum yeok} `child 3.\textsc{poss}{}-two', lit. `child its-two' {\textgreater} `second child'. 

Within the ordinal\is{ordinal numerals} possessive\is{possession} constructions, three areal patterns are discernible. The first pattern is that of the languages of Pantar and the Straits, where the possessive\is{possession} ordinal\is{ordinal numerals} construction includes an additional element specific for ordinals\is{ordinal numerals}. The second pattern is found in Central-East Alor, where ordinals\is{ordinal numerals} are also expressed like possessive\is{possession} constructions, but without including an additional ordinal\is{ordinal numerals} element. The third pattern is found in Kula\il{Kula} and Sawila\il{Sawila} in East Alor, where the ordinals\is{ordinal numerals} involve an applicative verb. We discuss the three patterns in turn.

In the languages of Pantar and the Straits, possessive\is{possession} constructions like those in \REF{bkm:Ref342649616} are the base for ordinal\is{ordinal numerals} constructions such as those in \REF{bkm:Ref342649632}, where the elements \textit{maing},\textit{ma} or \textit{mi} occur between the possessor\is{possession} prefix and the numeral. (Full paradigms of ordinal\is{ordinal numerals} constructions are presented in the Appendix.) 

\ea\label{bkm:Ref342649616}
\gllllll  {W Pantar\ilt{Western Pantar}}    aname    gai    bla\\
  Teiwa\ilt{Teiwa}      masar    ga-    yaf\\
  Kaera\ilt{Kaera}      masik    ge-    ma\\
  Blagar\ilt{Blagar}      mehal    {\textglotstop}e-    hava\\
  Adang\ilt{Adang}       nami    o-    bang\\
...
\gllllll
      {W Pantar\ilt{Western Pantar}}     aname  gai  maing  atiga \\  
      Teiwa\ilt{Teiwa}          masar  ga-  ma-  yerig \\
      Kaera\ilt{Kaera}          masik  ge-  mi-  tug \\
      Blagar\ilt{Blagar}         mehal {\textglotstop}e-  mi-  tue  \\
      Adang\ilt{Adang}          nami  o-  mi-  towo  {}-mi \\      
      { }            man \textsc{3.poss}  \textsc{ord-} three  -\textsc{ord} \\
\glt  `the third man'
\z


In Western Pantar\il{Western Pantar}, the ordinal\is{ordinal numerals} element is a free form \textit{maing}; in Teiwa\il{Teiwa}, Kaera\il{Kaera}, Blagar\il{Blagar}, and Adang\il{Adang} it is a bound morpheme (\textit{ma-} or \textit{mi-}). The ordinal\is{ordinal numerals} elements are formally similar to existing words in the respective languages: Western Pantar\il{Western Pantar} \textit{mayang} `to place', Teiwa\il{Teiwa}\textit{ma} `come, \textsc{obl}', and Kaera\il{Kaera}/Blagar\il{Blagar}/Adang\il{Adang} \textit{mi} `\textsc{obl' (}{\textless}pAP\il{proto-Alor-Pantar} *mai `come' and *mi `be in/on' \citealt{HoltonRobinsonTV}). Synchronically, the semantic and syntactic link between these free forms and the ordinal\is{ordinal numerals} markers is obscure. It may be that the ordinal\is{ordinal numerals} morphemes express notions that are (historically) related to notions of placement or location at a particular numeral rank. However, their position preceding the numeral does not parallel the position of verbs and oblique markers, which in AP languages always \textit{follow} their nominal complement. Note however, that the ordinals\is{ordinal numerals} in Adang\il{Adang} involve two identical morphemes: one preceding and one following the numeral. This might reflect an earlier structure where the ordinal\is{ordinal numerals} marker followed the numeral, paralleling the position of case markers and verbs.

The second areal pattern of ordinal\is{ordinal numerals} constructions is found in Central-East Alor, where ordinal\is{ordinal numerals} constructions are also possessive\is{possession} constructions but now without an ordinal\is{ordinal numerals} element included. Compare the constructions in \REF{bkm:Ref342649962} and \REF{bkm:Ref342309284}. The basic possessive\is{possession} construction in \REF{bkm:Ref342649962} includes a possessor\is{possession}, an alienable\is{alienability} possessive\is{possession} prefix and a possessum. In the ordinal\is{ordinal numerals} constructions in \REF{bkm:Ref342309284}, the ranked entity is the possessor\is{possession} of the numeral indicating the rank.

\ea%bkm:Ref342649962
\label{bkm:Ref342649962}
   Nominal possessive\ist{possession} construction in Central-East Alor \\
\langinfo{}{}{}\\
\glll  Kamang\ilt{Kamang}  lami  ge-  kadii \\  
     Abui\ilt{Abui}     neng  he-  fala  \\
...
\label{bkm:Ref342309284}
    Ordinal\ist{ordinal numerals} construction in Central-East Alor\\
\langinfo{}{}{}\\
\glll Kamang\ilt{Kamang}   lami  ge- su \\  
    Abui\ilt{Abui}     neng  he-  sua   \\
    { }  man  3\textsc{.poss-}  three\\
\glt  `the third man'
\z 


In East Alor, ordinal\is{ordinal numerals} structures that diverge from both these areal patterns are found in Kula\il{Kula} and Sawila\il{Sawila}. Kula\il{Kula} (Nick Williams, p.c. 2013) and Sawila\il{Sawila} ordinals\is{ordinal numerals} employ applicative verbs involving the cognate prefixes \textit{we-/wii-}, illustrated in \REF{bkm:Ref355275027} and \REF{bkm:Ref355277550}. In Kula\il{Kula} ordinals\is{ordinal numerals} this verb combines with a possessive\is{possession} structure, \REF{bkm:Ref358037937}. In Sawila\il{Sawila}, possessive\is{possession} constructions  are not used in ordinals\is{ordinal numerals}, \REF{bkm:Ref342737790}. 
...
In all AP languages, the ordinals\is{ordinal numerals} for `second' and higher are regularly derived. There is no limit in the creation of ordinals\is{ordinal numerals} on the basis of higher, morphologically more complex, cardinals\is{cardinal numerals}.

Some variation exists, however, in the expression of `first'. Adang\il{Adang} and Kamang\il{Kamang} form `first' by the regular process used for `second' and above. Teiwa\il{Teiwa} and Abui\il{Abui} use forms for `first' that are unrelated to the numeral `one', compare \REF{bkm:Ref342651040} (a-b) and \REF{bkm:Ref342651068} (a-b). In Teiwa\il{Teiwa}, the regular derivation from \textit{nuk} does not exist, \REF{bkm:Ref342651040}(b); in Abui\il{Abui}, it does exist, but has a different meaning (`the only/single/particular'), \REF{bkm:Ref342651068}(b). Western Pantar\il{Western Pantar} has two options to express `first'. One is to use the regular construction derived from \textit{(a)nuku} `one', as in \REF{bkm:Ref342651067}(a) while the other option is to use a different root \textit{ye} \REF{bkm:Ref342651067}(b) with an unclear etymology. There is a functional difference between Western Pantar\il{Western Pantar} ordinal\is{ordinal numerals} based on \textit{anuku} which is often used in predicative contexts (`you are the first'), and \textit{ye}, which is preferred in attributive contexts (`my first child'). 
...
In sum, the AP languages regularly derive ordinals\is{ordinal numerals} from numerals with a possessor\is{possession} morpheme, so that syntactically the ordinal\is{ordinal numerals} construction is a possessed\is{possession} nominal phrase. Apart from the third person possessor\is{possession} morpheme, which is used across the board, ordinals\is{ordinal numerals} vary in structure when we go from west to east. In the western languages (Pantar-Straits-West Alor) special morphemes are employed which may be etymologically related to free forms encoding locations, though synchronically, this relation is not transparent. In the eastern languages, ordinals\is{ordinal numerals} involve an applicative morpheme. At least three of the AP languages have an ordinal\is{ordinal numerals} `first' involving a root that is different from the cardinal\is{cardinal numerals} `one'. This is in line with the cross-linguistic tendency for languages with ordinals\is{ordinal numerals} unrelated to cardinals\is{cardinal numerals} to confine them to the lowest numerals \citep{StolzEtAl2013}.
...
Distributive numerals\is{distributive numerals} function to express notions such as `one by one' or `in groups of three'. AP languages create distributive\is{distributive numerals} numerals by reduplication\is{reduplication} of the cardinal\is{cardinal numerals} numeral, or a part of it. Cross-linguistically, reduplication\is{reduplication} is the most common strategy to form distributives\is{distributive numerals}: in about 33\% of the 251 languages in Gil's (2013) \nocite{Gil2013} sample, distributives\is{distributive numerals} are created in this way. As Gil points out, the reduplicative\is{reduplication} strategy is iconically motivated: repeated copies of the cardinal\is{cardinal numerals} correspond to multiple sets of objects.
...
\caption{Adang\ilt{Adang} cardinals\ist{cardinal numerals} and distributives\ist{distributive numerals}}
\label{tab:8:3}
%\label{}
\end{table}

\footnotetext{Synchronically, the vowel in the distributive\ist{distributive numerals} \textit{allo} has a distinct quality from the vowel in the cardinal\ist{cardinal numerals}.}
Across the AP family, the formation of distributives\is{distributive numerals} by reduplicating\is{reduplication} (parts of) cardinals\is{cardinal numerals} is a productive process. It applies not only to frequent or morphologically simple numerals such as `one' or `two', but also to less frequent and morphologically complex numerals like `27' in \REF{bkm:Ref342661452}, `201' in \REF{bkm:Ref342656299}, and `1054' in \REF{bkm:Ref342744393}. It must be noted that, while it is difficult to imagine a distributive\is{distributive numerals} context for numerals like these, speakers are able to mechanically derive their distributive\is{distributive numerals} form. 
...
\label{bkm:Ref342656299}
 ...
 Distributive\ist{distributive numerals} for `201' \\
\glllll  {} 100    2    \textsc{add}    \textsc{rdp}\~{}1\\  
 {W Pantar\ilt{Western Pantar}:} ratu    alaku    wali     ye\~{}ye        \\
 Teiwa\ilt{Teiwa}:    ratu    raq    rug    nuk\~{}nuk      \\
 Abui\ilt{Abui}:     aisaha     ayoku    wal    nuk\~{}nukda      \\
 Kamang\ilt{Kamang}:   ataak     ok     waal    no\~{}nok     \\  
\z
...
\label{bkm:Ref342744393}
 ...
 Distributive\ist{distributive numerals} for `1054'\\
\glllll   {}   1000    1  10  5    \textsc{add}  \textsc{rdp}\~{}4 \\      
   {W Pantar\ilt{Western Pantar}:}  (a)ribu   nuk   ke  yasing     wali  atu\~{}atu    \\
   Teiwa\ilt{Teiwa}:    ribu     nuk  qaar  yusan    rug  ut\~{}ut    \\
   Abui\ilt{Abui}:     rifi    nuku  kar  yeting    wal  buk\~{}bukna      \\
   Kamang\ilt{Kamang}:   ribu    nok  ataak  wesing    waal  bye\~{}biat     \\
\z


In sum, distributives\is{distributive numerals} are productively derived from cardinals\is{cardinal numerals} by reduplicating\is{reduplication} part of or the whole cardinal\is{cardinal numerals} base. In morphologically complex forms, the right-most element is the prosodic head and the reduplicative\is{reduplication} base. 

\subsubsection{Irregularities in distributive\ist{distributive numerals} formation}
\label{sec:8:Irregularities}
Exceptions to the regular derivations are mainly found in the formation of the morphologically complex low numerals `six' to `nine'. The irregularities include: (i) irregular segmental changes in reduplicated\is{reduplication} forms; (ii) irregular patterns of partial viz. full reduplication\is{reduplication}; and (iii) irregular choice of reduplicative\is{reduplication} base. 

Abui\il{Abui} shows the greatest amount of formal difference between its cardinal\is{cardinal numerals} and distributive\is{distributive numerals} numerals, as shown in Table \ref{tab:8:4}. The distributives\is{distributive numerals} are reduplicated\is{reduplication} verbal constructions: their verbal status is clear from the suffixes -\textit{da/-na/-ra} which encode light verbs and (continuative) aspect \citep{Kratochvil2007}. In Table \ref{tab:8:4}, the parts printed in bold show the irregular relation between Abui\il{Abui} cardinals\is{cardinal numerals} and the numeral morphemes used in distributives\is{distributive numerals}.
...
\caption{Abui\ilt{Abui} cardinals\ist{cardinal numerals} and distributives\ist{distributive numerals}}
\label{tab:8:4}
%\label{}
\end{table}

  In Kamang\il{Kamang} distributives\is{distributive numerals}, the reduplicant\is{reduplication} varies in shape. In the numerals `one' to `four' and `six', a morpheme with the shape (C)VV is reduplicated\is{reduplication}, while in the numeral `five' and the complex numerals built on it---`seven' through `nine'---the reduplicant\is{reduplication} has the shape CVCV. This is shown in \REF{bkm:Ref342656818}:

\ea%bkm:Ref342656818
\label{bkm:Ref342656818}
 
 Kamang\ilt{Kamang} distributive\ist{distributive numerals} numeral formation
\begin{tabular}{llll}            
\mytopline
...
Kamang\il{Kamang} has an irregular choice of reduplicative\is{reduplication} base. Compare the reduplicants\is{reduplication} (in bold-face) in the numerals `seven' to `nine' in \REF{ex:8:1236}. We see that Abui\il{Abui} reduplicates\is{reduplication} only the right-most numeral, resulting in word-internal reduplication\is{reduplication}, which is consistent with the regular distributive\is{distributive numerals} pattern in AP languages (Section \ref{sec:8:2.3.2}). By contrast, Kamang\il{Kamang} reduplicates\is{reduplication} the initial element \textit{wesing}. As main stress is on the final syllable of the numerals in Kamang\il{Kamang} just as it is in Abui\il{Abui}, we analyse this as a choice of reduplicative\is{reduplication} base in Kamang\il{Kamang} distributives\is{distributive numerals} which diverges from the overall pattern of AP languages. 

\ea
\label{ex:8:1236}
Reduplication\ist{reduplication} of base-5 numerals in Abui\ilt{Abui} and Kamang\ilt{Kamang}\\
\begin{tabular}{lll}
\mytopline
...
\langinfo{Abui}{}{}\\
 \gll Yeting  nu  ayoku-ng  ha-bel  maiye  he-pot  sua\\
  five  \textsc{spec}   two-\textsc{see } 3.\textsc{pat}{}-subtract  if  \textsc{3.alien\is{alienability}}{}-remainder  three \\
...
The strategy used in multiplication is variable. All languages start with the multiplicant, but its shape differs. In Western Pantar\il{Western Pantar} and Teiwa\il{Teiwa} it is an underived cardinal\is{cardinal numerals} followed by a demonstrative, while in Abui\il{Abui} it is a morphologically derived distributive\is{distributive numerals} (Section \ref{sec:8:2.3}). Examples \REF{bkm:Ref358043144} through \REF{bkm:Ref358043179}  illustrate `five times four is twenty':
...
Kamang\il{Kamang} expresses multiplication with an applicative verb derived from a cardinal\is{cardinal numerals} base by prefixing \textit{mi-}. (Compare Teiwa\il{Teiwa}, where the applicative derivation is used for fractions, see Section \ref{sec:8:3.5}). 
...
Note that the order of the verb relative to its complement `two' in \REF{bkm:Ref342664493}-\REF{bkm:Ref342746525} is unexpected, as it goes against the canonical AP object-verb order, found in subtraction (Section \ref{sec:8:3.2}). Note that the equivalent expression in Indonesian\il{Indonesian}/Malay\il{Malay} is \textit{sepuluh bagi dua (adalah)} \textit{lima} lit. `ten divide two (is) five', with verb complement order. It may be the case that the constructions in \REF{bkm:Ref342664493}-\REF{bkm:Ref342746525} are calques from Indonesian\il{Indonesian}/Malay\il{Malay}. 
...
In Kamang\il{Kamang}, fractions are verbs derived by prefixing \textit{wo-} `3.LOC' to the numeral base, as in \REF{bkm:Ref342746224}. In \REF{bkm:Ref342746258}, the derived verb is part of a resultative serial verb construction.
...
\label{sec:8:Summary}
The majority of cardinal\is{cardinal numerals} numerals in AP languages are morphologically complex expressions---most are compounds. These forms have quinary or decimal bases, though mathematical operations always employ a decimal base. No AP language has a numeral `null' or a word for `zero'---the absence of entities is expressed predicatively instead.

Ordinals\is{ordinal numerals} are derived from cardinals\is{cardinal numerals} by means of a third person possessor\is{possession} morpheme. Syntactically, ordinals\is{ordinal numerals} are possessive\is{possession} phrases where the ranked numeral is possessed\is{possession} by the ranked item. In the languages of Pantar, the Straits and West Alor, ordinal\is{ordinal numerals} constructions also contain a dedicated ordinal\is{ordinal numerals} morpheme; an applicative morpheme is used in the ordinals\is{ordinal numerals} of languages of Central and East Alor. 

Most languages derive distributives\is{distributive numerals} from cardinals\is{cardinal numerals} by reduplicating\is{reduplication} part or whole of the cardinal\is{cardinal numerals}. In complex forms, the right-most lexeme, which is the prosodic head of the compound, is taken as the base for the reduplication\is{reduplication}. This applies even to those forms that are synchronically morphologically opaque. Kamang\il{Kamang} is exceptional in that it reduplicates\is{reduplication} the left-most element of the compound rather than the prosodic head, and in Abui\il{Abui}, distributives\is{distributive numerals} and cardinals\is{cardinal numerals} are only indirectly related. 
...
\startappendix
\subsection{Ordinal\ist{ordinal numerals} and Distributive Numerals\ist{distributive numerals}} 

\begin{table}\centering
 \caption{Western Pantar\ilt{Western Pantar} ordinals\ist{ordinal numerals} in a construction with \textit{bla} `house' and \textit{aname} `person'}
\label{tab:8:5}
\begin{tabular}{lllll}
\mytopline
...
\begin{table}\centering
\caption{Teiwa\ilt{Teiwa} ordinals\ist{ordinal numerals} with \textit{yaf} `house' and \textit{uy} `person' }
\label{tab:8:6}
\begin{tabular}{lllll}
\mytopline
...
\footnotetext{Teiwa\ilt{Teiwa} \textit{ga-nuk} means `one from a group', \textit{ga-ma-nuk} is not a Teiwa\ilt{Teiwa} word.}

\begin{table}\centering
\caption{Kaera\ilt{Kaera} ordinals\ist{ordinal numerals} with \textit{ma} `house' and \textit{ui} `person'}
\label{tab:8:7}
\begin{tabular}{lllll}
\mytopline
...
\begin{table}\centering
\caption{Adang\ilt{Adang} ordinals\ist{ordinal numerals} with \textit{bang} `house' and \textit{nami} `person'}
\label{tab:8:8}
...
\begin{table}\centering
\caption{Abui\ilt{Abui} ordinals\ist{ordinal numerals} with \textit{fala} `house' and \textit{ama} `person'}
\label{tab:8:9}
\begin{tabular}{llll}
\mytopline
...
\begin{table}\centering
\caption{Kamang\ilt{Kamang} ordinals\ist{ordinal numerals} for \textit{kadii} `house' and \textit{alma} `person'}
\label{tab:8:10}
\begin{tabular}{llll}
\mytopline
...
\begin{table}\centering
\caption{Sawila\ilt{Sawila} ordinals\ist{ordinal numerals} with \textit{araasing} `house' and \textit{imyalara} `man' }
\label{tab:8:11}
\begin{tabular}{llll}
\mytopline
...
It is preferred to elicit the data for this questionnaire using words and constructions in the language of investigation as much as possible. The Malay\il{Malay} examples below are not given as prompts to be translated, but rather as additional background for you to help you steer a discussion in Malay\il{Malay}. Expressions containing numerals and ordinals\is{ordinal numerals}, and morphological derivations relating to numerals and ordinals\is{ordinal numerals} in the AP languages are expected to be quite different from what they are in Malay\il{Malay}. 

\subsubsection*{Tasks}
\begin{enumerate}
\item Ask a person to count in sequence from 1-20 and record this.
\item Elicit 1-100 on paper. Appendix 1: answer sheet.
\item Elicit higher cardinals\ist{cardinal numerals} 2000, 3000,..., 10.000. Appendix 2: answer sheet. 
...
\item Ordinals\ist{ordinal numerals}: Elicit 1\textsuperscript{st}{}-10\textsuperscript{th.} e.g., \textit{Saya lihat barisan anak di muka rumah. Yang pertama bernama... yang kedua... yang ketiga...} etc.
...
\item Check if there is a contrast in counting small animates versus large animates and animate vs. inanimate entities: 
  ...
  \item Inanimates 
    ...
    \end{itemize} 
  \end{itemize}
\item Note down the distribution of cardinals\ist{cardinal numerals} as part of NP (in `attributive' function), for example in a context like:
...
\item If there is variability, check if it is related to higher vs. lower cardinals\ist{cardinal numerals}. E.g. Malay\il{Malay} 
\begin{itemize}
...
\item Note down the distribution of cardinals\ist{cardinal numerals} as predicate (in `predicative' function), if they are used as such, e.g.:  
\begin{itemize}
...
\end{itemize}
\item If cardinals\ist{cardinal numerals} may be used in predicative function, can a higher numeral also be used as such? Note that this not generally possible in Malay\il{Malay}, where the predicative \textit{ber-} construction is not productively used with higher numerals: \textit{*Waktu itu kami} \textit{berduapuluh}\textit{.} Instead one would say \textit{Waktu itu kami duapuluh orang} `We were twenty at the time'. 

\begin{itemize}
\item Check e.g. 12, 15, 20, 35, 50, 76, 95. 
\end{itemize}
\item In Malay\ilt{Malay}, certain particular high cardinals\ist{cardinal numerals} do appear in the \textit{ber-} construction: \textit{Kami akan datang berseribu} `we will come (as) a (group of) thousand'. So perhaps a language does not treat all higher cardinals\ist{cardinal numerals} in the same way. 
\begin{itemize}
\item Check e.g. 1000, 2000, 100, 500, 1 000 000, 2 000 000
\end{itemize}
\item Can cardinals\ist{cardinal numerals} be used as elliptical for a fuller NP (subject or object): \textit{Mau berapa buah pisang? Saya mau} \textit{dua}\textit{(dua buah/ dua pisang)}
\item Can cardinals\ist{cardinal numerals} be used as abstract entities, e.g. in contexts like: 
\begin{itemize}
...
\item  \textbf{Reduplication\ist{reduplication} of cardinals\ist{cardinal numerals}:} Can numerals be reduplicated\ist{reduplication}? If so, give some examples in sentential context. 
\begin{itemize}
...
\item Does reduplicated\ist{reduplication} `one' have any special meaning? E.g. Bunaq \textit{uen\~{}uen} means `same, equal'; Kamang\ilt{Kamang} \textit{no-nok} `one by one'.
...
\end{itemize}
\item  Note down the \textbf{distribution of Ordinals\ist{ordinal numerals}}: 
\begin{itemize}
\item as part of NP: 
\begin{itemize}
...
\end{itemize}
\end{itemize}
\item  Are the ordinals\ist{ordinal numerals} etymologically clearly related to cardinals\ist{cardinal numerals}? e.g. Indonesian\ilt{Indonesian} ordinal\ist{ordinal numerals} \textit{pertama} is not derived from cardinal\ist{cardinal numerals} \textit{satu.}
\item  Are there any words that are used like ordinals\ist{ordinal numerals} but have no numeral or ordinal\ist{ordinal numerals} root?
...
\subsubsection{Higher cardinals\ist{cardinal numerals}}
...
2 & 2nd person\\
3 & 3rd person\\
alien & alienable\is{alienability}\\
AP & Alor-Pantar\\
appl & applicative \\
caus & causative\\
...
def & definite\\
dur & durative\\
ind & Indonesian\il{Indonesian}\\
loc & locative\\
N & noun\\
NP & noun phrase\\
num & numeral\\
ord & ordinal\is{ordinal numerals}\\
pat & patient\\
pl & plural\\
