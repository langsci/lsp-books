...
\section{Sound correspondences and reconstruction}
...
Evidence for *b in final position is based only on a single reconstruction for `wave'. In \citep{HoltonEtAl2012}, `fish', `tongue', `sugarcane', and `wave' were all reconstructed with a final *b, but with many of the cognates containing final vowels which we assumed were epenthetic. External evidence from Timor languages (see chapter 3 and \citet{SchapperEtAl}) has forced us to reconstruct those final vowels for `fish', `tongue', and `sugarcane'. It is possible that `wave' also had a final vowel in pAP, though we have insufficient evidence to reconstruct that at this time.
...
Final *k is retained in all languages except Blagar, Adang, Sawila, and Wersing, where it is lost entirely. Only two correspondence sets, `one' and `horn', support this reconstruction. Neither set has cognates in Sawila; however, final \textit{k }is rare in our Sawila data set, occurring in only two forms: \textit{werpa{\textlengthmark}k }`frog' and \textit{kispa{\textlengthmark}k }`earthworm' (both lexical innovations shared with Wersing). The correspondences for final *k can be difficult to tease apart from those for medial *k, since many languages reflect later vowel epenthesis or apocope. We take the presence of a geminate in Western Pantar to be diagnostic in this regard, since Western Pantar geminates do not occur word-finally. This criterion is admittedly problematic, since it is entirely possible that vowel epenthesis preceded gemination in Western Pantar. Furthermore, Western Pantar sometimes lacks cognates for relevant lexical items, as with `horn'.
...
\trs{*-ena}{give}&\trs{*mi}{(be) in/on}&\trs{*tiara}{expel}\\
\trs{*ga-}{\textsc{3}\textsc{sg}} & \trs{*mid}{climb} & \trs{*-tiari(n)}{close} (v.)\\
\trs{*ge-}{\textsc{3}\textsc{gen}} & \trs{*-mim}{nose} & \trs{*-tok}{stomach}\\
\trs{*gi-}{\textsc{3}\textsc{pl}} & \trs{*min(a)}{die} & \trs{*tukV}{short}\\
\trs{*ha-}{\textsc{2}\textsc{sg}} & \trs{*mis}{sit} & \trs{*-uaqal}{child}\\
\trs{*habi}{fish}&\trs{*mogol}{banana}&\trs{*-uari}{ear}\\
\trs{*haban}{village}&\trs{*mudi}{body hair}&\trs{*uasin}{tooth}\\
\trs{*hada}{fire, firewood} & \trs{*mudin}{plant} (v.) & \trs{*uku}{knee}\\
\trs{*hagur}{yawn}&\trs{*-muk}{horn}&\trs{*-wa}{mouth}\\
\trs{*hami}{breast}&\trs{*mVn}{rotten}&\trs{*wadi}{sun}\\
\trs{*has}{excrement} &  \trs{*na-}{1\textsc{sg}} & \trs{*wai}{blood}\\
\trs{*hasak}{empty}&\trs{*nai}{eat/drink}&\trs{*wai}{roof}\\
\trs{*hawar}{lime}&\trs{*nan(a)}{sibling (older)}&\trs{*war}{stone}\\
\trs{*hipar}{dream}&\trs{*nuk}{one}&\trs{*wata}{coconut}\\
\trs{*hu{\textlengthmark}ba}{sugarcane}&\trs{*oda}{throw}&\trs{*weli}{bathe}\\
\trs{*is(i)}{fruit}&\trs{*-ora}{tail}&\trs{*wur}{moon}\\

\end{tabular}
\label{bkm:Ref340670270}
\caption{Reconstructed pAP vocabulary}
...
\begin{figure}
% \includegraphics[width=5.3701in,height=2.5189in,width=\textwidth]{Ch2HoltonRobinsonAPhistoryMKprooftmp-img1.png}
...
\begin{figure}
% \includegraphics[width=6.2665in,height=2.5425in,width=\textwidth]{Ch2HoltonRobinsonAPhistoryMKprooftmp-img2.png}
...
\begin{figure}
% \includegraphics[width=5.7756in,height=4.7in,width=\textwidth]{Ch2HoltonRobinsonAPhistoryMKprooftmp-img3.jpg}
...
\begin{figure}
% \includegraphics[width=5.1252in,height=2.9937in,width=\textwidth]{Ch2HoltonRobinsonAPhistoryMKprooftmp-img4.png}
...
\stepcounter{footnote}\footnotetext{A form *balin `axe' was reconstructed to pAP in \citep{HoltonEtAl2012}, but we now recognize that this is an Austronesian loan, probably from Alorese \textit{baling}. }
\stepcounter{footnote}\footnotetext{This form has metathesized. }
\stepcounter{footnote}\footnotetext{This form has metathesized. }
\stepcounter{footnote}\footnotetext{Denotes `chicken'.}
\stepcounter{footnote}\footnotetext{This reconstruction is strikingly similar to the Austronesian (proto-Malayo-Polynesian) form \textit{*maRi} `come', which is irregularly reflected as \textit{ma} or \textit{mai }in many Austronesian languages in the region (cf. Mambai (Timor) \textit{ma}, Kambera (Sumba) \textit{mai}). However, similar reflexes are not found in Lamaholot or Alorese, the immediate Austronesian neighbors of the Alor-Pantar languages.}
\stepcounter{footnote}\footnotetext{Denotes `traditional dance'.}
\stepcounter{footnote}\footnotetext{Denotes `bow, bend'.}
\stepcounter{footnote}\footnotetext{This form was not reconstructed to pAP in \citep{HoltonEtAl2012} because it is not attested in Alor languages. However, based on its presence in Timor languages (see chapter 3), we now reconstruct it to pAP.}
\stepcounter{footnote}\footnotetext{Denotes `not quite dry'.}
\stepcounter{footnote}\footnotetext{Denotes `eat' in Tw, Nd, WP, Ab, Km, `eat/drink' in Ke, Bl, Sw, and `drink' in Ad, Kl, Ki, We.}
\stepcounter{footnote}\footnotetext{This form exhibits metathesis. }
\stepcounter{footnote}\footnotetext{This form was not reconstructed to pAP in \citep{HoltonEtAl2012} because of its limited distribution. However, based on its presence in Timor languages (see chapter 3), we now reconstruct it to pAP.}
\stepcounter{footnote}\footnotetext{Denotes `burn, of land'.}
\stepcounter{footnote}\footnotetext{Denotes `clothing louse', with metathesis. }
\stepcounter{footnote}\footnotetext{Note similarity with proto-Austronesian *isi{\textglotstop} `contents', indicating that this may be a loan.}
\stepcounter{footnote}\footnotetext{Denotes `meat'.}
\stepcounter{footnote}\footnotetext{This form was not reconstructed to pAP in \citep{HoltonEtAl2012} because it is not attested in any Pantar language. However, based on its presence in Timor languages (see chapter 3), we now reconstruct it to pAP.}
\stepcounter{footnote}\footnotetext{Reflexes of *p\{i,u\}nV typically encompass the meanings `hold' and `grab' with the difference depending on the prefixation of the verb.}
\stepcounter{footnote}\footnotetext{Sw has \textit{wuni} `hold' and \textit{puni }`hit'.}
\stepcounter{footnote}\footnotetext{We has \textit{woi}\textit{{\ng}} `hold' and \textit{poi}\textit{{\ng}} `hit'.}
\stepcounter{footnote}\footnotetext{This form was not reconstructed to pAP in \citep{HoltonEtAl2012} due to its limited distribution. However, based on its presence in Timor languages (see chapter 3), we now reconstruct it to pAP.}
\stepcounter{footnote}\footnotetext{Denotes `maggot'.}
\stepcounter{footnote}\footnotetext{This form is generally part of a compound when meaning `chin'. It seems to have historically meant `mouth'. It is retained with that meaning in Ke, Ki, Ab, and Km. In Tw, Nd, WP, Bl, Ad, Sw, and We, the form is only retained as part of a compound meaning `chin'.}
\stepcounter{footnote}\footnotetext{Denotes `new sprout'.}
\stepcounter{footnote}\footnotetext{Reflexes of *tapai encompass the meanings `pierce', `stab' `sew', `plant in the ground', and `pound rice'. }
\stepcounter{footnote}\footnotetext{Denotes `on top'.}
\stepcounter{footnote}\footnotetext{Denotes `thatch'.}
\stepcounter{footnote}\footnotetext{This form was not reconstructed to pAP in \citep{HoltonEtAl2012} because it is not attested in the eastern languages. However, based on its presence in Timor languages (see chapter 3), we now reconstruct it to pAP.}
\stepcounter{footnote}\footnotetext{Denotes `piece, chunk'.}
\stepcounter{footnote}\footnotetext{Denotes `short piece, cutting'.}
\stepcounter{footnote}\footnotetext{Denotes `dance'.}
\stepcounter{footnote}\footnotetext{Denotes `dance'.}
\stepcounter{footnote}\footnotetext{This form has lost the initial syllable. }
\stepcounter{footnote}\footnotetext{Denotes `sweet'.}
\stepcounter{footnote}\footnotetext{Denotes `shine, burn' (cf. \textit{was }`sun').}
...
\stepcounter{footnote}\footnotetext{This form has metathesized. }
\stepcounter{footnote}\footnotetext{Denotes `beat, strike (drum)'.}
\stepcounter{footnote}\footnotetext{This form was not reconstructed to pAP in \citep{HoltonEtAl2012} because it is not attested in the eastern languages. However, based on its presence in Timor languages (see chapter 3), we now reconstruct it to pAP.}
\stepcounter{footnote}\footnotetext{Tw\textit{ laman }`follow, walk along (e.g. a path)'. WP \textit{lama }shares this sense and is likely a borrowing from Tw, which explains the lack of gemination in the WP form. }
\stepcounter{footnote}\footnotetext{This may be an Austronesian loan. Note proto-Malayo-Polynesian *timuR `southeast monsoon'  \citep{BlustEtAl2010}.}
\stepcounter{footnote}\footnotetext{This form was not reconstructed to pAP in \citep{HoltonEtAl2012} because it is not attested in any eastern language. However, based on its presence in Timor languages (see chapter 3), we now reconstruct it to pAP. }
