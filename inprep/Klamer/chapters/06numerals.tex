...
{C \& E Alor} & Abui & {\itshape nuku} & {\itshape ajoku} & {\itshape sua} & {\itshape buti} & \textit{jeti}\textit{{\ng}}\\
 & Kamang & {\itshape nok} & {\itshape ok} & {\itshape su} & \textit{biat} \footnotemark{} & \textit{iwesi}\textit{{\ng}}\\
 & Sawila & (\textit{sundana}){\dag} & {\itshape jaku} & {\itshape tuo} & (\textit{ara:si:ku}) & \textit{jo:ti}\textit{{\ng}}\\
 & Kula & (\textit{sona}) & {\itshape jakwu} & {\itshape tu} & (\textit{arasiku}) & {\itshape jawatena}\\
 & Wersing & {\itshape no} & {\itshape joku} & {\itshape tu} & (\textit{arasoku}) & \textit{weti}\textit{{\ng}}\\
\mybottomline
\end{tabular}
{
...
\stepcounter{footnote}\footnotetext{Liquid-stop metathesis has occurred in Blagar-Bama \textit{akur} `two', but not in other Blagar dialects.}
\stepcounter{footnote}\footnotetext{In Abui and Kamang, the vowels in `four' display some irregular patterns. For Kamang, it is necessary to posit the following metathesis: Proto-Alor Pantar *buta {\textless} *bita {\textless} biat.} 
...
In the north-central Alor language Kamang, the formation of `six' differs between dialects, as set out in \REF{ex:6:5}. The Atoitaa dialect has a base-five compound of `five [plus] one', while the dialect of Atoitaa reflect p\textsc{AP} *talam `six'. 
...
The languages of central-east Alor construct `seven' to `nine' with an additive base-five system that is synchronically transparent. That is, speakers of these languages readily parse their numerals `seven' to `nine' as being composed of the synchronic numeral `five' followed by `two', `three', or `four', as set out in \REF{ex:6:6}. 
...
The subtractive basis for the formation of Straits-West-Alor numerals `seven' through `nine' is evident from the remnants of reflexes of proto-AP *(a)tiga `three', *araqu `two' and *nuk `one' in the final syllables of modern forms, as set out in \REF{ex:6:8}. Bolding indicates the matching strings of segments. 
...
In \REF{ex:6:10} we set out the formatives found in Kui `seven' to `nine'. We can see that Kui has replaced the proto-Straits-West-Alor subtractive numerals for `seven' and `nine' with additive base-five numerals [5 2], [5 4]. The numeral \textit{tadusa} `eight' follows a different pattern, apparently being built from two morphemes: (i) the first element \textit{tad-} appears to reflect the subtractive morpheme *tur- ({\textless} pre-pSWA *tukari) used in forming pSWA *turarok `eight' (see \REF{ex:6:9}), and (ii) the second element \textit{usa} is the Kui numeral `four' ({\textless} pAP *buta `four'). 
...
In \REF{ex:6:11} we set out the formatives found in Western Pantar `seven' to `nine'. We can see that proto-Straits-West-Alor subtractive numeral *tukarinuk `nine' have been replaced by an innovative, but still subtractive form composed of the numeral `one' denoting the subtrahend and the lexical verb `take away' signalling the subtraction. The numerals `seven' and `eight' follow a different, innovative pattern in which \textit{be- \~{} bet-}, reflecting *{\texthtb}uti- as also used in the formation of proto-Straits-West-Alor *{\texthtb}utitoga `seven', is combined with `two' and `three' to form `eight' and `nine' respectively. 
...
&  & {`ten'} & {`twenty'} & {`thirty'}\\
\midrule 
{Pantar} & Western Pantar & \textit{ke}\footnotemark{}\textit{anuku} & {\itshape ke alaku} & {\itshape ke atiga}\\
...
{Straits} & Blagar\-Bama & {\itshape qar nuku} & \textit{qar} \textit{akur} & \textit{qar} \textit{tuge}\\
...
{West Alor} & Kabola & {\itshape kar nu} & \textit{kar} \textit{ho(}\textit{{\textglotstop}}\textit{)olo} & \textit{kar} \textit{towo}\\
...
{C\&E Alor} & Abui & {\itshape kar nuku}  & \textit{kar} \textit{ajoku} & \textit{kar} \textit{sua}\\
...
The additive operator is not used to combine decades, hundreds or thousands with each other, as illustrated with `1999' in several languages in \REF{ex:6:15}. 
...
Not all modern AP languages reflect the reconstructed operator. Kaera and Blagar- Dolabang have apparently related operators, while Teiwa has a unique form \textit{rug}. In eastern Alor languages, the additive operators (Kula \textit{aris}\textit{{\textbari}}\textit{{\ng}}, Sawila \textit{garisi{\ng}}, Wersing \textit{weresi{\ng}}) are the result of shared borrowing from an Austronesian (Austronesian) language of Timor, most likely Tokodede. The Austronesian languages of eastern Timor invariably use an (ad)verb `more' with a variant of the form /geresin/ as the additive operator in numerals. Examples are provided in \REF{ex:6:16}.   
...
{Flores} 	&  Rongga 	&  {(e)sa} 			&  {{\textturnr}ua} 	&  {telu} 		&  {wutu} 				&  {lima} 		\\	
			&  Ende 	&  {sa} 			&  {zua} 		&  {tela} 		&  {wutu} 				&  {lima} 		\\	
			&  Ngadha 	&  {esa} 			&  {zua} 		&  {telu} 		&  {vutu} 				&  {lima} 		\\	
			&  Nage 	&  {esa} 			&  {{\texthtd}ua} 	&  {telu} 		&  {wutu} 				&  {lima} 		\\	
			&  K\'eo {\dag} &  {ha{\textglotstop}esa} 	&  {{\textglotstop}esa rua}&  {{\textglotstop}esa tedu}&  {{\textglotstop}esa wutu} 	&  {{\textglotstop}esa dima} \\	
			&  Lio 		&  {{\textschwa}sa} 		&  {rua} 		&  {t{\textschwa}lu} 	&  {sutu} 				&  {lima} 		\\	
{Lembata} 	&  Kedang* 	&  {{\textgreater}ude{\textglotstop}}&  {sue} 		&  {t{\ae}lu} 		&  {{\textgreater}apa{\textglotstop}} 	&  {leme} 		\\	
...
\Tree [.PAP [.PP [.PCP [.PCEP Kaera Sar ] Teiwa ] Deing ] [.PSWA \qroof{\parbox{1cm}{\vspace{.1cm}Blagar\\Reta\\Kabola\\Adang\\Hamap\\Klon}}.{}  Kui\raisebox{.4cm}[-2cm]{\hspace{-.2cm}?} West{ }Pantar\raisebox{.4cm}[-2cm]{\hspace{-1.1cm}?\hspace{1cm}} ]!\qsetw{3.3cm} \parbox{.1cm}{Abui} [.PCEA Kamang [.PEA Wersing [.PEAM Sawila Kula ]  ]  ]  ] 
...
\mytopline
Abui (AP) 	&  \citet{Kratochvil2007}, Schapper fieldnotes 2010 	  \\
Adang (AP, Pitungbang dialect) 	&  Robinson fieldnotes 2010 	 \\
Alorese (AN) 	&  \citet{Klamer2011} 	  \\
Amarasi (AN) 	&  \citet{BaniEtAl2011} 	 \\
...
Bunaq (TAP, Lamaknen) 	&  \citet{Schapper2010} 	\\
Bunaq (TAP, Manufahi) 	&  Schapper fieldnotes 2007 	 \\
Dadu'a (a.k.a. Galoli) (AN) 	&  \citet{Penn2006} 	  \\
Dhao 	&  \citet{GrimesEtAl2008} 	 \\
Deing (AP) 	&  B. Volk fieldnotes 2008 	 \\
Ende (AN) 	&    \citet{AokiEtAl1993} 	  \\
...
Kedang (AN) 	&  \citet{Samely1991} 	 \\
Kemak (AN, Atabai dialect) 	&  Klamer fieldnotes 2002 	\\
K\'eo (AN) 	&  \citet{Baird2002} 	 \\
Klon (AP) 	&  \citet{Baird2008} 	\\
Komodo (AN) 	&  \citet{Verheijen1982} 	\\
Kui (AP) 	&  Baird fieldnotes 2003, Holton fieldnotes 2010 	\\
Kula (AP) 	&  Holton fieldnotes 2010, Nicholas Williams p.c. 2011 	  \\
\mybottomline
\end{tabular}
...
\mytopline
 &  & {`ten'} & {`twenty'} & {`thirty'}\\
\midrule 
{Pantar} & {Western Pantar} & {\itshape ke anuku} & {\itshape ke alaku} & {\itshape ke atiga}\\
 & {Deing} & {\itshape qar nuk} & {\itshape qar raq} & {\itshape qar atig}\\
 & {Sar} & {\itshape qar nuk} & {\itshape qar raq} & {\itshape qar tig}\\
 & {Teiwa} & {\itshape qa:r nuk} & {\itshape qa:r raq} & {\itshape qa:r jerig}\\
 & {Kaera} & {\itshape xar nuko} & {\itshape xar raxo} & {\itshape xar tug}\\
