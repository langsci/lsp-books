...
Nominal plurality is expressed by the plural word \textit{maru(ng)}, \REF{ex:9:15}.\footnote{  \textit{Marung} has cognate forms in three AP languages: Klon \textit{maang}, Kula \textit{araman} (with liquid nasal metathesis) and Sawila \textit{maarang} (Schapper and Huber, ms.)} The use of numerals is illustrated in \REF{ex:9:16}, and \REF{ex:9:17}-\REF{ex:9:18} show that numeral and plural word do not co-occur in a single NP.
