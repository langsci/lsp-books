...
We have not attested an AP language with a number word for `null' or `zero'. The absence of entities is rather expressed predicatively, using a word meaning `(be) empty', such as Teiwa \textit{hasak} in \REF{bkm:Ref355363568}.
\todo[inline]{fix broken crossref by inserting corresponding label bkm:Ref355363568 between {\textbackslash}REF\{\}}\footnote{{}  Compare proto-Alor-Pantar *hasak (Holton and Robinson, this volume), reflected in Western Pantar \textit{hakkas}, Kaera \textit{isik}, Abui \textit{taka}, Kamang \textit{saka}.} In the Teiwa idioms in \REF{bkm:Ref355363568}, a subject precedes a nominal predicate that is headed by the place pronoun \textit{i} `it.(place)', so that absent entities are expressed as ``X is (an) empty place(s)'', compare (\REF{bkm:Ref355363568} (a-b).
...
In the languages of Pantar and the Straits, possessive constructions like those in \REF{bkm:Ref342649616} are the base for ordinal constructions such as those in \REF{bkm:Ref342649632}, where the elements \textit{maing},\textit{ma} or \textit{mi} occur between the possessor prefix and the numeral. (Full paradigms of ordinal constructions are presented in the Appendix.) 

\ea\label{bkm:Ref342649616}
\gllllll  {W Pantar}    aname    gai    bla\\
  Teiwa      masar    ga-    yaf\\
  Kaera      masik    ge-    ma\\
...
The second areal pattern of ordinal constructions is found in Central-East Alor, where ordinal constructions are also possessive constructions but now without an ordinal element included. Compare the constructions in \REF{bkm:Ref342649962} and \REF{bkm:Ref342309284}. The basic possessive construction in \REF{bkm:Ref342649962} includes a possessor, an alienable possessive prefix and a possessum. In the ordinal constructions in \REF{bkm:Ref342309284}, the ranked entity is the possessor of the numeral indicating the rank.
...
Some variation exists, however, in the expression of `first'. Adang and Kamang form `first' by the regular process used for `second' and above. Teiwa and Abui use forms for `first' that are unrelated to the numeral `one', compare \REF{bkm:Ref342651040} (a-b) and \REF{bkm:Ref342651068} (a-b). In Teiwa, the regular derivation from \textit{nuk} does not exist, \REF{bkm:Ref342651040}(b); in Abui, it does exist, but has a different meaning (`the only/single/particular'), \REF{bkm:Ref342651068}(b). Western Pantar has two options to express `first'. One is to use the regular construction derived from \textit{(a)nuku} `one', as in \REF{bkm:Ref342651067}(a) while the other option is to use a different root \textit{ye} \REF{bkm:Ref342651067}(b) with an unclear etymology. There is a functional difference between Western Pantar ordinal based on \textit{anuku} which is often used in predicative contexts (`you are the first'), and \textit{ye}, which is preferred in attributive contexts (`my first child'). 
...
Distributive numerals follow the noun or pronoun they modify, as illustrated in \REF{bkm:Ref342655549}-\REF{ex:8:1235}. Distributives can modify different clausal arguments; for example, an actor subject in \REF{bkm:Ref342655549} and \REF{bkm:Ref342738939} or a patient object in \REF{bkm:Ref342655553}.
...
In some AP languages distributives may float outside the NP to a position adjacent to the verb; an example is Adang, \REF{ex:8:1235}. The exact restrictions and possibilities of such constructions across the AP languages remain a topic for future research; here we focus on the morphological shape of the distributives.
...
\end{table}
  ...
  In Kamang distributives, the reduplicant varies in shape. In the numerals `one' to `four' and `six', a morpheme with the shape (C)VV is reduplicated, while in the numeral `five' and the complex numerals built on it---`seven' through `nine'---the reduplicant has the shape CVCV. This is shown in \REF{bkm:Ref342656818}:
...
Kamang has an irregular choice of reduplicative base. Compare the reduplicants (in bold-face) in the numerals `seven' to `nine' in \REF{ex:8:1236}. We see that Abui reduplicates only the right-most numeral, resulting in word-internal reduplication, which is consistent with the regular distributive pattern in AP languages (Section \ref{sec:8:2.3.2}). By contrast, Kamang reduplicates the initial element \textit{wesing}. As main stress is on the final syllable of the numerals in Kamang just as it is in Abui, we analyse this as a choice of reduplicative base in Kamang distributives which diverges from the overall pattern of AP languages. 
...
The strategy used in multiplication is variable. All languages start with the multiplicant, but its shape differs. In Western Pantar and Teiwa it is an underived cardinal followed by a demonstrative, while in Abui it is a morphologically derived distributive (Section \ref{sec:8:2.3}). Examples \REF{bkm:Ref358043144} through \REF{bkm:Ref358043179}  illustrate `five times four is twenty':
...
In Kamang, fractions are verbs derived by prefixing \textit{wo-} `3.LOC' to the numeral base, as in \REF{bkm:Ref342746224}. In \REF{bkm:Ref342746258}, the derived verb is part of a resultative serial verb construction.
