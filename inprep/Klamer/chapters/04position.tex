...
where available. }
Prior to this work most authors assumed a connection to Trans-New Guinea languages, based primarily on evidence from pronominal paradigms \citep{Ross2005}. However, several other plausible hypotheses have been proposed, which we shall examine in this chapter.The Timor-Alor-Pantar (TAP) languages are surrounded on all sides by Austronesian languages, with the nearest Papuan (non-Austronesian) language located some 800 km distant.\footnote{The extinct language of Tambora, known only from nineteenth century wordlists, was spoken some 650 km west of Pantar, and it is presumed to have been non-Austronesian \citep{Donohue2007-Tambora}.} Some putative relatives of the TAP family are shown in Figure \ref{tab:4:1} below.
...
Nor are these features present in proto-North Halmahera (pNH), shown in Table \ref{tab:4:2}.
...
\caption{pNH consonants \citep[after][]{Wada1980}}
...
\footnotetext{Note that the pTNG apical stop *t may have had a flap or trill allophone \citep[273]{Pawley2001}.}
...
\section{Pronominal evidence} \ref{sec:4:3}
...
\mytopline
& \rm pAP & \rm pTNG & \multicolumn{2}{c}{\rm pNH} \\
...
pTNG *{\ng}gi `\textsc{2pl}'  & pAP *gi- `\textsc{3pl}' \\
...
The structural features of the pronominal systems are compared in Table \ref{tab:4:table_pronominal_features}. It is apparent that the AP pronominal system as a whole has relatively little in common with TNG and NH.
...
\mybottomline
\end{tabular}
\caption{Summary of AP, TNG, and NH pronominal features} 
...
\mytopline
&\rm TAP (Capell)&\rm TAP (revised)&\rm NH (Capell)&\rm NH (revised)\\
\midrule  
...
`smell'&{\textglotstop}amuhu&Teiwa \textit{min}, Kaera \textit{mim-, }Nedebang \textit{mini}, Blagar \textit{miming}, Adang \textit{muning}, Klon \textit{moin}, Kui \textit{mun}, Wersing \textit{muing}, Makasae \textit{amuh}, Makalero \textit{kamuhata}, also pTAP *\textit{-mVN} `nose'&*ami&\\
`stone'&hele&pTAP *\textit{war}&*helewo&{\rm Galela} \textit{teto}, {\rm Tabaru} \textit{madi}\\
...
\caption[pNH forms with TAP equivalents A-M]{pNH forms \citep[after][]{Wada1980} with TAP equivalents (after Schapper et al., this volume), sorted alphabetically by pNH form. A double dagger {\ddag} indicates a pNH form which is not in Wada or a pAP form which is not reconstructed at the level of pTAP.\footnotemark{}} 
\label{table_pNH}
...
\footnotetext{In the pTAP / pAP reconstructions, V stands for an unidentified vowel, and N stands for an unidentified nasal. The other reconstructed consonants have their values as laid out in Table \ref{tab:4:1}. The vowels, while very tentative, are assumed to have their IPA values. } 
...
\stepcounter{footnote}\footnotetext{Schapper et al. (chapter 3) reconstruct pTAP *uaQal, where *Q is ``\emph{\textmd{\textup{a putative postvelar stop for which we have only very weak evidence''. We prefer to render this as *uaqal, showing more transparently the value we believe this consonant would have had. }}}}
...
\begin{tabular}{l>{\it}l>{\it}l>{\it}l}
\mytopline
&\rm personal&\rm possessive&\rm pAP\\ 
\midrule
