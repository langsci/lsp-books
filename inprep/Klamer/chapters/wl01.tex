...
\mytoprule
{Language}{\dag} & {ISO\newline639-3} & {Alternate   Name(s)} & {Pop.}\footnotemark{\ddag} & {References \par (selected)}\\
\midrule 
...
\label{tab:1:1}
{\dag}{The abbreviations in brackets are used to refer to the languages in the historical comparative chapters by \citet{HoltonRobinsonTVhistory,HoltonRobinsonTVposition} and \citet{SchapperEtAlTVtimor}.} 
{\ddag}{Population estimates from fieldworker and/or from the published source given; starred (*) estimates from \citet{LewisEtAl2013}; an empty cell indicates that no number has been reported.}
{\dag\dag}{This figure is from census data \citep{BadanPusat2005}.} 
\end{table}
\clearpage 
