...
%%JW: not really good examples (would need one with water, sand, etc.), not sure the following on mass/count nouns is really relevant!!
Note that there is no formal distinction between countable and mass nouns in \PS, as illustrated by the example in \REF{uncountableEx1}, in which the words for ‘flour’, ‘sugar’ and ‘food’ are all inflected for plural. %\It{biergo} ‘meat’ is \PL\ and refers to the meat from two moose. 
...
\Transl{what is the name of the person who you are talking with?}{} \CorpusE{110521b1}{040}
...
Furthermore, the noun in a postpositional phrase occurs in the genitive case, %UM: revise, add sources, or DELETE footnote: \footnote{Historically, postpositional phrases stem from a figurative use of possession relationships concerning spatial relations; see Section \ref{postpositions} for more details.} 
as in \REF{gen2}.
\ea\label{gen2}
...
\ea\label{acc1}
\glll	dä virtiv válldet \Bf{giehpajt} ja \Bf{ribrev} ja \Bf{dagarijt} ulgos\\ %
	dä virti-v vállde-t giehpa-jt ja ribbre-v ja dagari-jt ulgos\\
...
Finally, familial relations can also be expressed using an illative construction. In such cases, the ‘ego’ of the family relation is in illative, as in \REF{ill5}.
\ea\label{ill5}
...
\z
In this example, the illative nominal is a pronoun, but it is likely the case that full nouns are possible in this function as well, although there are no such tokens in the corpus.
...
\z
\ea\label{elat2a}
\glll	nå gåsse dija älgidä \Bf{Örnvikast} vuodjet vadnásav\\
	nå gåsse dija állge-dä Örnvika-st vuodje-t vadnása-v\\ %vánás(str)??
...
\footnote{The \WLP’s wordlist indicates that the word \It{ájnát} can also be used to express ‘without’, but the documentation corpus does not provide any tokens of this.} 
...
\glll	vuodja dugu \Bf{goulle}\\
	vuodja dugu goulle\\
	swim\BS\Sc{3sg.prs} like fish-\Sc{nom.sg}\\\nopagebreak
\Transl{(he) swims like a fish}{} \CorpusE{110413a}{052}
\z
Finally, it should be pointed out that essive is not particularly common, and in the \PS\ corpus, tokens for this case are only found in elicitation sessions. In summary, there is not enough data to come to any definitive conclusions concerning the status of essive in current \PS\ usage. %UM: deleted:, and further study is necessary.
...
\PS\ has a number of portmanteau suffixes expressing the case and number. %that a noun is in. %UM: ‘that a noun is in’ streichen
...
In \Sc{nom.pl} and \Sc{gen.sg}, the \It{-h} suffix is optional in \PS\ (and therefore appears in parentheses in Table \vref{nounSuffixes}).%, but has only become so over the course of the last few generations of speakers.
...
The \Sc{com.sg} suffix has two allomorphs: \It{-jn} and \It{-jna}, which seem to be in free variation in the \PS\ corpus, and not determined phonologically. 
...
\begin{table}\centering
\caption{The case/number paradigm for the noun \It{bärrgo} ‘meat’}\label{meatParadigm}
...
\end{tabular}
\end{table}
Here, two groups of patterns are manifest: the forms for \Sc{nom.sg}, \Sc{ill.sg} and \Sc{ess} show one pattern, while all other case/number combinations exhibit the other pattern. This alignment of the choice of stem allomorphs is prevalent throughout \PS\ noun paradigms whenever stem allomorphy is a part of a noun’s inflectional paradigm. 
...
xː	&:&x		&/kolːe/	&:&/kole/	& ‘fish’\\%ll-l
	&&		&\It{guolle}&&\It{guole}&\\\cline{4-7}
	&&		&/naːʰpːe/	&:&/naːʰpe/	& ‘milking bowl’\\%bm-m
	&&		&\It{náhppe}&&\It{náhpe}&\\\hline
xːy	&:& xy	&/ɲaːrːka/	&:&/ɲaːrka	/& ‘cape’ (geog.)\\%rrg-rg
	&&		&\It{njárrga}&&\It{njárga}&\\\hline
xy	&:&y		&/etno/	&:&/eno/	& ‘river’\\%bm-m
	&&		&\It{edno}	&&\It{eno}&\\\hline
xyz	&:& xz	&/vaːjpmo/&:&/vaːjmo/	& ‘heart’\\%\hline%jbm-jm
	&&		&\It{vájbmo}&&\It{vájmo}&\\\hline
ʰx	&:&x		&/tɔʰpe/	&:&/tɔpe/	& ‘house’\\%bm-m
...
ɛ	&:&e		&/pɛkːa/	&:&/pekːa/	& ‘wind’\\%l
	&&		&\It{bägga}	&&\It{biegga}	& \\\hline%l
u͡a	&:&o		&/lu͡akːta/	&:&/lokta/	& ‘bay’\\%
...
The following sections present the four inflectional noun classes based on a preliminary analysis of the \PS\ corpus; it is possible that, with more research, more noun classes may result, or that the present classes may need revision. Because each \PS\ noun paradigm consists of seventeen inflectional forms, most of the data on which these classes are based comes from elicitation sessions, as it is far beyond realistic for a single, non-native-speaker linguist to collect a sufficiently large natural (i.e., un-elicited, spontaneous) spoken language corpus which includes all inflectional forms for a large variety of nouns.
...
The header \It{grade alignment} refers to the choice of stem allomorph in \Sc{nom.sg} versus \Sc{nom.pl}, when consonant gradation is relevant for a specific noun paradigm: the value ‘str-wk’ indicates that \Sc{nom.sg} is marked by the strong grade and \Sc{nom.pl} by the weak grade, while ‘wk-str’ is the opposite, inverted pattern. 
...
\begin{table}\centering
\caption{Summary of noun classes}\label{nounClassSummary}
\begin{tabular}{c c c}
		&\It{grade} 	&\It{class suffix}		\\
\It{class}	&\It{alignment}	&\It{in} \Sc{nom.sg}\\\dline
I		&str-wk		&-a/á/o/å		\\\hline
II		&wk-str		&-Vj	\\\hline
III		&wk-str		&-	\\\hline
\end{tabular}
...
The gradation pattern and class marking suffixes for Class I are summarized in Table \vref{NclassIsuffixes}. Here, V stands for the vowel which comprises the suffix for each class (i.e., \It{a} for Class Ia, \It{á} for Class Ib, \It{o} for Class Ic, and \It{å} for Class Id).
\begin{table}\centering
...
\Sc{abess}	&wk		& -V-			&wk		& -V-		\\\dline%\cline{2-3}
			&\MC{2}{c|}{\It{C-grad}}	&\MC{2}{c|}{\It{class suffix}}	\\\cline{2-5}
\Sc{ess}	&\MC{2}{c|}{str}	&\MC{2}{c|}{-V-}\\\hline%\hline
\end{tabular}
\end{table}
...
\begin{table}\centering
\caption{The class-marker suffix and case/number paradigm for the Class Ia noun \It{luakkta} ‘bay’}\label{bayParadigm}
\begin{tabular}{ |r | c | c | }\hline
			&\MC{2}{c|}{\It{number}}\\
\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\dline
...
%%CLASS Ib
\begin{table}\centering
\caption{The class-marker suffix and case/number paradigm for the Class Ib noun \It{mánná} ‘child’}\label{childParadigm}
\begin{tabular}{ |r | c | c | }\hline
			&\MC{2}{c|}{\It{number}}\\
\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\dline
...
%%CLASS Ic
\begin{table}\centering
\caption{The class-marker suffix and case/number paradigm for the Class Ic noun \It{bäbbmo} ‘food’}\label{foodParadigm}
\begin{tabular}{ |r | c | c | }\hline
			&\MC{2}{c|}{\It{number}}\\
\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\dline
...
%%CLASS Id
\begin{table}\centering
\caption{The class-marker suffix and case/number paradigm for the Class Id noun \It{skåvvlå} ‘school’}\label{schoolParadigm}
\begin{tabular}{ |r | c | c | }\hline
			&\MC{2}{c|}{\It{number}}\\
\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\dline
...
The inflectional paradigm for the word \It{guolle} ‘fish’ is provided as an example in Table \vref{fishParadigm}.
For nouns with consonant gradation and/or umlaut in Class Ie, \Sc{nom.sg}, \Sc{ill.sg} and \Sc{ess} are in the strong grade and have \It{ä} or \It{ua/uä} in V1 position, while other case/number slots have the weak grade and \It{ie} or \It{uo} in V1 position.


\begin{table}\centering
\caption{The class-marker suffix and case/number paradigm for the Class Ie noun \It{guolle} ‘fish’}\label{fishParadigm}
\begin{tabular}{ |r | c | c | }\hline
			&\MC{2}{c|}{\It{number}}\\
\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\dline
...
\Sc{abess}	&wk		& -e-			&wk		& -e-	\\\dline%\cline{2-3}
			&\MC{2}{c|}{\It{C-grad}}	&\MC{2}{c|}{\It{class suffix}}	\\\cline{2-5}
\Sc{ess}	&\MC{2}{c|}{str}	&\MC{2}{c|}{-e-}\\\hline%\hline
%\begin{tabular}{ |r | c | c | }\hline
%			&\MC{2}{c|}{\It{number}\\
%\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\dline
...
In order to include another example of a Class Ie noun affected by \It{-j}-suffix vowel harmony (but without consonant gradation), the paradigm for \It{vágge} ‘valley’ is provided in Table \vref{valleyParadigm}. %\marginpar{såhke ‘birch’ not affected}
\begin{table}\centering
\caption{The class-marker suffix and case/number paradigm for the Class Ie noun \It{vágge} ‘valley’}\label{valleyParadigm}
\begin{tabular}{ |r | c | c | }\hline
			&\MC{2}{c|}{\It{number}}\\
\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\dline
...
\begin{table}\centering
\caption{The Class III consonant gradation pattern and inflectional noun class suffixes}\label{NclassIIsuffixes}
\begin{tabular}{ |r || c | c || c | c | }\hline
			&\MC{4}{c|}{\It{number}}\\
			&\MC{2}{c||}{\Sc{singular}}	&\MC{2}{c|}{\Sc{plural}}	 \\%\dline
\It{case}	&\It{C-grad}&\It{class suffix}	&\It{C-grad}&\It{class suffix	} \\\dline
...
\Sc{abess}	&str		& -V-			&str		& -V-	\\\dline%\cline{2-3}
			&\MC{2}{c|}{\It{C-grad}}	&\MC{2}{c|}{\It{class suffix}}	\\\cline{2-5}
\Sc{ess}	&\MC{2}{c|}{wk}	&\MC{2}{c|}{-V-}\\\hline%\hline
%\begin{tabular}{ |r | c | c | }\hline
%			&\MC{2}{c|}{\It{number}\\
%\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\dline
...
\item{the gradation pattern (when relevant) is inverted}
\end{itemize}
Furthermore, this class consists of two subclasses (IIIa and IIIb). 


\subsubsection{Class IIIa}\label{NclassIIIa}
This, the more common Class III subclass, exhibits a \Sc{nom.sg} form which lacks a class marker and ends in a closed syllable; in this case, the stem-final consonant is thus the word-final consonant. 
The word \It{sabek}\footnote{In adhering to \PS\ orthographic conventions, word-final /k/ is spelled with <k>, while intervocalic /k/ is spelled <g>.} 
‘ski’ is provided in Table \vref{skiParadigm} %on page \pageref{skiParadigm} 
as a example for this subclass. %, in which the class suffix for \Sc{nom.sg} forms a closed syllable. 
...
\begin{table}\centering
\caption{The class-marker suffix and case/number paradigm for the Class IIIa noun \It{sabek} ‘ski’}\label{skiParadigm}
\begin{tabular}{ |r | c | c | }\hline
			&\MC{2}{c|}{\It{number}}\\
\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\dline
...
\begin{table}\centering
\caption{The class-marker suffix and case/number paradigm for the Class IIIa denominal noun \It{guolátj} ‘little fish’}\label{fishDIMparadigm}
\begin{tabular}{ |r | c | c | }\hline
			&\MC{2}{c|}{\It{number}}\\
\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\dline
...
\begin{table}\centering
\caption{The class-marker suffix and case/number paradigm for the Class IIIb noun \It{bena} ‘dog’}\label{dogParadigm}
\begin{tabular}{ |r | c | c | }\hline
			&\MC{2}{c|}{\It{number}}\\
\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\dline
...
\Sc{abess}	&str		& ?			&str		& ?	\\\dline%\cline{2-3}
			&\MC{2}{c|}{\It{C-grad}}	&\MC{2}{c|}{\It{class suffix}}	\\\cline{2-5}
\Sc{ess}	&\MC{2}{c|}{?}	&\MC{2}{c|}{?}\\\hline%\hline
%\begin{tabular}{ |r | c | c | }\hline
%			&\MC{2}{c|}{\It{number}\\
%\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\dline
%\Sc{nom}	& -Vj			& -V		\\\hline%\cline{2-3}
...
While the possessive suffixes go back to Proto-Saami \citep[73]{Sammallahti1998}, they seem to have nearly fallen out of use in contemporary \PS, and are only attested in three recordings from the corpus. These examples from the corpus are presented initially, and a description follows. 

While there are technically nine tokens of possessive pronouns in the corpus, these nine tokens can be grouped into two identical sets, so that effectively only two examples are evident. Specifically, there are three tokens of \It{áhttjes} ‘my father’ in nominative case by one speaker in two different recordings, and three tokens of the parallel construction \It{mammaset ja pappaset} ‘your mother and your father’ by another speaker in one recording. An example from the first speaker is provided in \REF{possSuffix1}, and an example\footnote{The example in \REF{possSuffix5} is essentially identical to the other utterance with four tokens of these same noun stems with possessive suffixes (pit100703a.034).} 
from the second speaker in \REF{possSuffix5}.
\ea\label{possSuffix1}%JW: where’s the finite verb here?!?
\glll	áhttjes dá lä gähtjamin jus gävdnij aktak, nag gietjokmiesse\\
	áhttje-s dá lä gähtja-min jus gävdni-j aktak nagin gietjokmiesse\\
	father-\Sc{1sg.poss\BS nom.sg} then be\BS\Sc{3sg.prs} look-\Sc{prog} if exist-\Sc{3sg.pst} any some unmarked\_calf\BS\Sc{nom.sg}\\%pappa-1sg.poss nu titta-3sg.prog om finns nån någan omärk-kalv
...
\glll	nå dä hålij, nå hälset del mammaset ja pahppaset\\
	nå dä håli-j nå hälse-t del mamma-set ja pahppa-set\\
