...
\PS\ nouns inflect\is{inflection} for singular and plural in all grammatical cases except the essive\is{essive} and possibly the abessive\is{abessive} case. Dual\is{dual} is not a relevant category for nouns, despite being an integral category in verb morphology %, cf. Chapter \ref{verbs}
...
\subsection{Nominative case}\label{nominative}\is{nominative}
...
\subsection{Genitive case}\label{genitive}\is{genitive}
...
Furthermore, the nominal complement in a postpositional phrase\is{postpositional phrase} occurs in the genitive case, as the noun \It{gåde} ‘hut’ in \REF{gen2}.
\ea\label{gen2}
...
\subsection{Accusative case}\label{accusative}\is{accusative}
...
\z
The accusative can also mark nouns functioning as a clause-level temporal adverbial phrase\is{adverbial phrase} denoting a period of time, %are normally also inflected for the \ACC\ case, 
...
\subsection{Illative case}\label{illative}\is{illative}
...
\subsection{Inessive case}\label{inessive}\is{inessive}
...
\subsection{Elative case}\label{elative}\is{elative}
...
Similarly, in a copula clause\is{copula clause}, the elative case can mark a noun whose referent is a source of pain, as in \REF{elat6}.
\ea\label{elat6}
\glll	mån lev {åjvest}\\
	mån le-v åjve-st\\
...
\subsection{Comitative case}\label{comitative}\is{comitative}
...
\subsection{Abessive case}\label{abessive}\is{abessive}
...
\subsection{Essive case}\label{essive}\is{essive}
...
\z
However, while my main consultant accepted constructions like in \REF{ess3}, her initial response normally consisted of nearly the same construction, only with the noun in nominative\is{nominative} case, as in \REF{ess4}. 
\ea\label{ess4}
...
The nominal suffixes marking case and number are listed in Table \vref{nounSuffixes}. Note that the status of the abessive\is{abessive} suffixes is unclear, including whether they inflect for number, as discussed in \SEC\ref{abessiveProblematic}. Nouns in essive\is{essive} case do not inflect for number.
...
\subsection{Problematic case/number marking in abessive case}\label{abessiveProblematic}\is{abessive}
%1. problematic
