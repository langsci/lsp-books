...
The current chapter first describes the morphological categories number (§\,\ref{numberNouns}) and case (§\,\ref{case}) in a general way in order to provide a background for the variables discussed in §\,\ref{NumCaseNouns} on morphological case and number marking. % depending on the nominal categories case and number. %and phonologically motivated vowel harmony. 
%Different nouns behave in different ways and can therefore be divided into
A description of the inflectional class markers and the resulting inflectional classes for nouns is given in §\,\ref{nounClasses}.\footnote{Cf. §\,\ref{nDerivation} for derivational morphology creating nouns.} % of Chapter \ref{derivMorph}.
...
§\,\ref{NumCaseNouns} on number and case marking %and §\,\ref{nounInflectionClasses} on inflectional classes 
...
%in a variety of ways which are covered in §\,\ref{NumCaseNouns}; this allows at least ?how?many? inflectional classes to be posited (see §\,\ref{nounInflectionClasses}). Pronouns also inflect for case (cf. Chapter \ref{pronouns}). 
...
Furthermore, the nominal complement in a postpositional phrase occurs in the genitive case, as the noun \It{gåde} in \REF{gen2}.
\ea\label{gen2}
...
% is used in a general stative locative meaning, as in \REF{whatMoreDidYouDoAhkkabakkte} and \REF{SalvoCreekInValley}. To indicate more detailed information on stative position, post positions are normally used (see §\,\ref{?}). 
...
%In possessive copula clauses, the possessor is in this case as in \REF{SaamiHasDog}.\footnote{Cf. §\,\ref{possCopClause} on the possessive copula clause}
\ea\label{iness3}
...
The essive case (glossed as \ESSs) generally marks predicative nouns functioning as complements of certain verbs such as \It{sjaddat} ‘become’, as in \REF{ess1a} and \REF{ess1b}, and \It{gähtjoduvvat} ‘be called’, as in \REF{ess2}. Note that nouns in the essive case do not inflect for number. 
...
\begin{tabular}{ r  x{35mm}  p{35mm}  }
			&\MC{2}{c}{\It{number}}			\\
\It{case}	& \Sc{singular} 	&\MC{1}{c}{\Sc{plural}}		 \\\hline
\Sc{nom}	&  - 				& \MC{1}{c}{- (\TILDE\ -h)}	\\%\cline{2-3}
\Sc{gen}	&  - (\TILDE\ -h)	&  \MC{1}{c}{-j}			\\%\cline{2-3}
\Sc{acc}	&  -v				&  \MC{1}{c}{-jt}			\\%\cline{2-3}
\Sc{ill}		&  -j			&  \MC{1}{c}{-jda}	\\%\cline{2-3}
\Sc{iness}	&  -n				& \MC{1}{c}{ -jn}		\\%\cline{2-3}
\Sc{elat}	&  -st				&  \MC{1}{c}{-jst}		\\%\cline{2-3}
\Sc{com}	&  -jn(a)			&  \MC{1}{c}{-j}			\\%\cline{2-3}
\Sc{abess}	& \MC{2}{l}{-dak, -daga, -gat, -gahta, -ahta}\\\hline%\cline{2-3}% \It{-dak}, \It{-daga}, \It{-gat}, \It{-gahta} and \It{-ahta}. 
		&\MC{2}{c}{\It{no number distinction}}	\\
\Sc{ess}	& \MC{2}{c }{-n}				\\\hline%\hline
...
Several of the nominal inflectional suffixes, considered by themselves, are homophonous: %constitute synchretic\index{homophony} sets:%; these are listed in Figure \vref{homoNounSuffixes}.
%\begin{figure}
\begin{itemize}
...
%\item{\It{-Ca} for \Sc{nom.pl} and \Sc{gen.sg} in Class III nouns\footnote{Cf. §\,\ref{nounClasses} for inflectional classes for nouns; note that \It{C} stands for the final consonant of a Class III stem.}}
...
\Sc{ill.sg/gen.pl/com.pl}	&\Sc{iness.pl/com.sg}&\Sc{ess/iness.sg}	&\Sc{nom.pl/gen.sg}	&\It{gloss}	\\\hline
almatjij				& almatjijn			& almatjin			& almatja(h)		& ‘person’	\\
...
%Some examples of homophonous noun pairs are listed in Table \vref{syncreticInflFormPairExamples}. See §\,\ref{nounStemAllomorphy} and \ref{nounUmlaut} as well as Table \vref{nounSuffixesWithStemAlts} below for a more detailed description of such syncretism.
\begin{table}\centering
...
\Sc{nom.pl/gen.sg}	&\Sc{gen.pl/com.pl}	&\Sc{iness.pl/com.sg}&\It{class}&\It{gloss} \\\hline
...
%			&\MC{2}{c}{\It{number}}			&	\\
\It{case}	&\It{j}-VH	& \SGs	&\It{j}-VH	& \PLs& \It{case} \\\hline
...
In addition to the suffixes described above, most nouns are also marked for case and number by non-linear stem allomorphy (cf. §\,\ref{morphophonology}). % alternations of stem consonants and vowels. 
...
Because \Sc{nom.sg}, \Sc{nom.pl} and \Sc{gen.sg} lack suffixes completely,\footnote{As mentioned in §\,\ref{nominalSuffixes} above, there is an optional \It{-h} suffix marking \Sc{nom.pl} and  \Sc{gen.sg}.} 
...
\Sc{nom}	& bärrgo			& biergo		\\%\cline{2-3}
\Sc{gen}	& biergo			& biergoj		\\%\cline{2-3}
\Sc{acc}	& biergov			& biergojd		\\%\cline{2-3}
\Sc{ill}	& bärrgoj			& biergojda	\\%\cline{2-3}
\Sc{iness}	& biergon			& biergojn		\\%\cline{2-3}
\Sc{elat}	& biergost			& biergojst	\\%\cline{2-3}
\Sc{com}	& biergojn			& biergo		\\%\cline{2-3}
\Sc{abess}& biergodak		& biergodahta	\\%\cline{2-3}
...
In summary, the inflectional paradigm for \It{bärrgo} is characterized by both consonant gradation and umlaut in the stem, %\footnote{As described in §\,\ref{}, umlaut only occurs in stems with consonant gradation, but consonant gradation can occur without umlaut.} %JW: NOT true, cf. bägga-biegga (wind)
...
%While consonant gradation is described in detail in §\,\ref{Cgrad}, Table \vref{CGpatterns} provides some examples of such stem consonant alternations in nouns. The examples provided show minimal pairs differing only in the choice of stem allomorph (here, the \Sc{nom.sg} form vs. the \Sc{nom.pl} form for nouns in class I are presented). 
...
%Umlaut in \PS\ is described in more detail in §\,\ref{}. 
...
%The feature ‘\jvh’ (abbreviated \It{j-VH}) indicates whether vowel harmony in V1 and V2 vowels exists in the presence of certain case/number suffixes with a \It{-j-} element (cf. §\,\ref{jSuffixHarmony} below). 
...
and is therefore dealt with first in §\,\ref{NclassI}, %Because Class I are very similar, they are dealt with together in the first section below (§\,\ref{NclassI2III}), 
...
\Sc{nom}	&str		& -V			&wk		& -V		\\%\cline{2-3}
\Sc{gen}	&wk		& -V			&wk		& -V-		\\%\cline{2-3}
\Sc{acc}	&wk		& -V-			&wk		& -V-		\\%\cline{2-3}
\Sc{ill}		&str		& -V-			&wk		& -V-		\\%\cline{2-3}
\Sc{iness}	&wk		& -V-			&wk		& -V-		\\%\cline{2-3}
\Sc{elat}	&wk		& -V-			&wk		& -V-		\\%\cline{2-3}
\Sc{com}	&wk		& -V-			&wk		& -V-		\\%\cline{2-3}
...
While there do not appear to be many words in Class II, the data in the corpus is ultimately inconclusive. %
...
\begin{tabular}{r  c  c}
			&\MC{2}{c}{\It{number}}\\
\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\hline
\Sc{nom}	& sabek			& sabeg-a		\\%\hline%\cline{2-3}%
...
\begin{tabular}{r  c  c}
			&\MC{2}{c}{\It{number}}\\
\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\hline
\Sc{nom}	& vanás			& vadnás-a		\\%\hline%\cline{2-3}
...
\begin{tabular}{r  c  c}
			&\MC{2}{c}{\It{number}}\\
\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\hline
\Sc{nom}	& guolátj			& guolátj-a		\\%\hline%\cline{2-3}
...
\MR{2}{*}{\Sc{ess}}	&\MC{2}{c}{\It{C-grad}}	&\MC{2}{c}{\It{class suffix}}	\\%\cline{2-5}
	&\MC{2}{c}{n/a}	&\MC{2}{c}{n/a}\\\hline%\hline
%\begin{tabular}{ r  c  c |}
%			&\MC{2}{c|}{\It{number}\\
%\It{case}	& \Sc{singular}	& \Sc{plural}	 \\\hline
%\Sc{nom}	& -Vj			& -V		\\\hline%\cline{2-3}
...
\begin{figure}[h]\centering
∑ \PLUS\ class-marker \PLUS\ possessive-case/number%\fbox{∑ \PLUS\ class-marker \PLUS\ possessive-case/number}
