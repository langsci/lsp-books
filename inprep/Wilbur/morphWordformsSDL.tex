...
Derivational morphology is commonly used to create nouns, verbs, and, to a lesser extent, adjectives and adverbs. %\footnote{\Red{Historically, at least attributive adjectives were probably derived as well, but synchronically, this is no longer the case; cf. Section \ref{notePredNounsAdjs}.}} 
...
The present section only provides an overview of these various morphological phenomena. 
Because morphological behavior varies between the word classes, it is described in more detail individually in the relevant word class chapters. The current section is divided into Section \ref{linearMorphology} on linear morphology and Section \ref{morphophonology} on non-linear morphological processes. %(consonant gradation, umlaut and morphologically-triggered vowel harmony).
...
The general linear morphological structure of \PS\ words has derivational suffixes attaching to a root before inflectional suffixes occur on the resulting stem, as illustrated by \marginpar{make sure Figure \ref{linearMorphStructure} and Table \ref{summaryMorphCats} aren’t directly above one another}Figure \vref{linearMorphStructure}.
...
These alternations come in pairs of stem allomorphs that differ quantitatively or qualitatively. % or in both respects. %in the consonants in the consonant center. 
Quantitative differences are alternations between a geminate consonant %\footnote{For more on such geminates, see Section \ref{geminateCs}.} 
...
For quantitative alternations, the form with more phonological material is called the \Bf{strong grade}\index{strong grade} (abbreviated ‘str’), while the form with less phonological material is referred to as the \Bf{weak grade}\index{weak grade} (abbreviated ‘wk’). In cases of qualitative differences, the form with a preaspirated segment is considered to be in the strong grade, while the plain alternation is in the weak grade. 
%Some examples to help illustrate this can be found below, and Table \vref{CgradPatternSummary} on page \pageref{CgradPatternSummary} summarizes the various patterns and the attested alternations. 

A summary of \korr{020}consonant center gradation patterns found in the corpus is provided in Table \vref{CgradPatternSummary}; here, \It{x}, \It{y} and \It{z} each represents a consonant phoneme in the consonant center, while geminates and contrastive preaspirated segments are marked additionally by <ː> and <ʰ>, respectively. %Note that there are various ways to organize the attested consonant gradation patterns. For instance, strictly speaking, the second group (ʰx:\Div ʰx) can be considered a subset of the first group (x:\Div x). However, due to …?  I have chosen to highlight the 
...
The minimal pairs in \REF{CgradEx2a} and \REF{CgradEx2b} are examples of quantitative consonant gradation patterns in which a phoneme present in the first form is absent in the second form. The consonant gradation alternations illustrated here are /tn\Div n/ and /jpm\Div jm/, respectively, and correspond to the patterns x\Div y and xyz\Div xz in Table \vref{CgradPatternSummary}. 
...
Finally, the example in \REF{CgradEx3} shows a qualitative consonant gradation pattern. %in which a phoneme present in the first form is absent in the second form. 
...
\It{vágge}		&\It{vágge}&\\
valley\BS\Sc{nom.sg}	& valley\BS\Sc{gen.sg}	&	\\
\end{tabular}
...
\Tn{\begin{tabular}{c c c}
			&south	&north	\\
/ʧaː\Bf{pː}a/	&/ʧaː\Bf{pː}e/	&/ʧaː\Bf{b}e/\\
...
\begin{figure}\centering
\begin{tabular}{c|x{40pt} | x{40pt}|c}\cline{2-3}
&	\It{A}			& \It{B}			&\\
&	ɛ \ARROW\ e	& u͡a \ARROW\ o	&\\\cline{2-3}
\end{tabular}
...
The allomorph /ɛ/ from pattern \It{A} is found in the same paradigmatic slot as /u͡a/\footnote{Note that /u͡a/ has an allomorph [u͡ɛ] triggered by purely phonological vowel harmony; cf. Section \ref{Vua}.} 
from pattern \It{B}, just as the allomorphs /e/ and /o/ also correspond to the same paradigmatic slots. 
...
The term \Bf{vowel harmony} (VH) refers to regressive phonological assimilation concerning the place of articulation of the V1 vowel of a stem in the context of certain V2 vowels.\footnote{Cf. Section \ref{prosodicDomains} on prosodic positions, including V1 and V2 positions.} 
...
\MC{3}{|c|}{\It{VH pattern}}	&\It{nouns}	&\It{verbs}		\\\dline	
...
\MR{2}{*}{nouns}&\MR{2}{*}{class II}&\Sc{gen.pl, acc.pl, ill.pl, iness.pl,}	\\%\hline
...
In \REF{vhEx1}, an example is shown of vowel harmony in the Class II noun \It{guolle} ‘fish’, as it alternates between /o/ in the V1 vowel of the \Sc{nom.sg} form, and /u/ in the \Sc{nom.pl} form.
\ea\label{vhEx1}%
\Tn{\begin{tabular}{c c}
...
words in \PS, and grouping such words based on that behavior, a total of seven word classes can be distinguished. These can be divided into two general categories containing generally \It{open} word classes and \It{closed} word classes, and are listed in Table \vref{wordClassList}. 
...
verbs		&tense/mood/person/number,		&head of a verb complex	\\%\hline
			&non-finite forms (negation, aspect)	&					\\\hline
adjectivals		&number for predicate adjectives	&head of an adjectival phrase	\\\hline%, but can head an NP in elliptic constructions\\\hline
...
%numerals		&specifies count of an NP						& none\\\hline%part of ADJ
%verbal particles	&complement? to VC head					& none\\\hline%part of ADV
