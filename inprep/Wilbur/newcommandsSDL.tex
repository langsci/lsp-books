...
\newcommand{\ARROW}{→}%%dieser dicke Pfeil ➜ wird nicht von der LSP-Font unterstützt: %\newcommand{\ARROW}{{\fontspec{DejaVu Sans}➜}}
\newcommand{\DARROW}{↔︎}%%DoubleARROW
\newcommand{\BULLET}{•}%
...
\newcommand{\Transl}[2]{\trans \mbox{‘#1’ \Tn{#2}}}%new transl tier in gb4e;
\newcommand{\TranslLong}[2]{\trans ‘#1’ \Tn{#2}}%new transl tier in gb4e when free translation particularly long
\newcommand{\TranslMulti}[2]{\trans \mbox{\hspace{10pt}‘#1’ \Tn{#2}}}%new transl tier in gb4e for a dialog to be included under a single example number
\newcommand{\TranslLongMulti}[2]{\trans \hspace{10pt}‘#1’ \Tn{#2}}%new transl tier in gb4e for a dialog when free translation is particularly long
...
\newcommand{\DerivExamX}[7]{\Tn{\begin{tabular}{lcl}\It{#1}&\ARROW&\It{#3}\\#2&&#4\\\end{tabular}\hfill\pbox{.3\textwidth}{\hfill‘#5’\\\hbox{}\hfill\hyperlink{pit#6}{{\small\textnormal[pit#6]}}}
%\index{Z\Red{rec}!\Red{pit#6}}\index{Z\Red{utt}!\Red{pit#6.#7}}
}}
