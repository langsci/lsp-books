...
Adverbs compose an open word class and are defined by their ability to head an adverbial phrase\is{adverbial phrase}; they can be further divided into two main groups: 
\begin{itemize}
...
This adverbializing suffix triggers the weak\is{weak grade} consonant grade\is{consonant gradation}, when applicable. Two examples from the corpus are provided in \REF{derivedADVsEx1} and \REF{derivedADVsEx2} below.
%\pagebreak
...
\subsubsection{The question marker \It{gu}\TILDE\It{gus}}\label{QpartWordform}\is{question marker}
In several Saami languages, including closely related Lule Saami, a grammatical unit often referred to in the literature as a ‘question particle’ is used to mark polar interrogative\is{polar interrogative} clauses.\footnote{Cf. North Saami \It{-go} \citep[cf.][90]{Svonni2009} and Lule Saami \It{-ga/-k/-ge} \citep[cf.][94-94]{Spiik1989}, which are cognate with \PS\ \It{gu}\TILDE\It{gus}. According to \citet[245]{Sammallahti1998}, the question marker was originally borrowed from Finnish. Skolt Saami also has a question marker \It{-a} \citep[cf.][319-320]{Feist2010}, which is not cognate.} 
...
\section{Adpositions}\label{adpositions}\is{adposition}
Adpositions in \PS\ constitute a closed class of words that are defined syntactically by their ability to head an adpositional phrase\is{adpositional phrase} (abbreviated ‘PP’ as these are either postpositional\is{postpositional phrase} or prepositional phrases\is{prepositional phrase}). 
...
\subsection{Postpositions}\label{postpositions}\is{postposition}
...
Postpositions are complemented by NPs in the genitive\is{genitive} case. 
...
\subsection{Prepositions}\label{prepositions}\is{preposition}
With the exception of \It{dugu} ‘like’, which governs a noun in either the essive\is{essive} or the nominative\is{nominative} case (cf. \SEC\ref{essive}), 
...
NPs\is{nominal phrase}, APs\is{adjectival phrase} and verbs\is{verb} can be connected to another phrase of the same type by a conjunction; however, it is not clear from the data whether PPs\is{postpositional phrase} or AdvPs\is{adverbial phrase} can be connected. 
