...
%\item{particles in Section \ref{particles}}
\item{interjections in Section \ref{interjections}}
\end{itemize}
The information provided here is of a preliminary nature due to limited data in the corpus, and stands to gain much from future research. %Sections \ref{conjunctions} through \ref{interjections} deal with postpositions, conjunctions, particles and interjections, in that order. %UM suggested moving this to list


\section{Adverbs}\label{adverbs}
\Bf{Adverbs} compose an open word class and are defined by their ability to head an adverbial phrase; they can be further divided into two main groups: 
\begin{itemize}
...
Section \ref{derivedADVs} deals with the former, while Section \ref{lexicalADVs} deals with the latter. 

\subsection{Derived adverbs}\label{derivedADVs}
The data in the corpus is very limited concerning derived adverbs, but at least one derivational affix seems to exist which derives an adverb from an adjective: the suffix \It{-git}, as illustrated by Table \vref{derivedADVsTable}; this adverbializing suffix triggers the weak consonant grade, when applicable. Two examples from the corpus are provided in \REF{derivedADVsEx1} and \REF{derivedADVsEx2} below.
\begin{table}\centering
\caption{Derived adverbs and their adjectival roots}\label{derivedADVsTable}
\begin{tabular}{|ccc||c|}\hline
\It{\ATTRs\ adjective}	&\ARROW&\It{adverb}	&\It{gloss}	\\\dline
várogis	&\ARROW&várogit		& ‘careful(ly)’	\\\hline
buorre	&\ARROW&buoragit		& ‘good/well’	\\\hline
%suojmas	&\ARROW&suojmagit	& ‘slow’	\\\hline
%\it  adverb 	&\it adjective	&\it gloss	\\\dline
...
\footnotetext{Note that \It{ganska} is a nonce borrowing from Swedish; cf. Sw. \It{ganska} ‘quite’.}
\z
...
\begin{tabular}{l l}
\It{lexical adverb}&\It{gloss}\\\hline
aj			& ‘also, too’	\\
ber\TILDE bar	& ‘only’	\\
del		& ‘obviously’	\\
dä			& ‘then’	\\
...
\glll	buhtsu mielkest ijtjen ber vuostajt dága\\
	buhtsu mielke-st ittj-in ber vuosta-jt dága\\
	reindeer\BS\Sc{gen.sg} milk-\Sc{elat.sg} \Sc{neg}-\Sc{3pl.pst} only cheese-\Sc{acc.pl} make\BS\Sc{conneg}\\\nopagebreak
%\TranslLong{they didn’t just make cheese from reindeer milk}{(lit.: from reindeer milk they didn’t make only cheeses)}	\CorpusLink{080708_Session03}{080708\_Session03}{001}
...
\glll	gajk vuorasumos saddje'l vanj dát urrum dulutjist\\
	...
	all old-\Sc{superl} place\BS\Sc{nom.sg}=be\BS\Sc{3sg.prs} definitely \Sc{dem}-\Sc{prox}-\Sc{nom.sg} be-\Sc{prf} old\_days-\Sc{elat.sg}\\\nopagebreak
...
A further lexical adverb is \It{gal} ‘actually’, which can be used to emphasize a contradiction or surprise, as in \REF{particleEx6}. The interjection \It{nä} ‘no’, a borrowing from Swedish,\footnote{< Sv. \It{nej} ‘no’; cf. local dialect pronunciation [nɛː].} 
...
\glll	\Tn{A:} udtju sáme gielav danne sagastit?\\
	...
	{} be\_allowed\BS\Sc{2sg.pst} Saami\BS\Sc{gen.sg} language-\Sc{acc.sg} there speak-\Sc{inf}\\\nopagebreak
\TranslMulti{were you allowed to speak the Saami language there?}{}\\
\glll	\Tn{B:} nä, ij gal, ittjiv åtjo\\
	{} nä ij gal i-ttjiv åtjo\\
	{} no \Sc{neg}\BS\Sc{3sg.prs} actually \Sc{neg}-\Sc{1sg.pst} be\_allowed\BS\Sc{conneg}\\\nopagebreak
...
In several Saami languages, including closely related Lule Saami, a grammatical unit typically referred to in the literature as a ‘question particle’ is used to mark polar interrogative clauses.\footnote{Cf. North Saami \It{-go} \citep[cf.][90]{Svonni2009} and Lule Saami \It{-ga/-k/-ge} \citep[cf.][94-94]{Spiik1989}, which are cognate with \PS\ \It{gu}\TILDE\It{gus}. According to \citet[245]{Sammallahti1998}, the question marker was originally borrowed from Finnish. Skolt Saami also has a question marker \It{-a} \citep[cf.][319-320]{Feist2010}, which is not cognate.} 
...
\z
\ea\label{Qpart2}
\glll	aná gus dån naginav, mujtojt?\\
	aná gus dån nagina-v mujt-o-jt\\
...
\Bf{Adpositions} in \PS\ constitute a closed class of words that are defined syntactically by their ability to head a adpositional phrase (abbreviated ‘PP’ as these are either postpositional or prepositional phrases). 
Postpositions, which are clearly preferred over prepositions, are covered in Section \ref{postpositions}. The limited data on prepositions, \korr{045}which, with on exception, can all be used as postpositions as well, are described in Section \ref{prepositions}. 
...
It is possible that other postpositions also exist, but were not attested to in the corpus; however, postpositions remain a closed class, as new postpositions cannot be created.
\begin{table}\centering%\resizebox{1\textwidth}{!} {
...
\begin{tabular}{l l}
%			&\it approx. \\
\It{postposition}&\It{translation equivalent}\\\hline
baddjel	& ‘above’ \\%cf. pit100324.25m51s
birra & ‘about; around’ \\
duogen & ‘behind’ \\%cf. pit100404.045
gaskan	& ‘between’ \\%cf. pit100324.49m19s
guoran & ‘next to; near’ \\%cf. pit0906_Ahkajavvre_ES.004
lahka	& ‘near’ \\%cf. pit080702DSb
nala	& ‘upon, up, towards’ \\
nanne / nan & ‘on’ \\
%baltan & ‘next to; beside ’ \\%in the corpus?!?
sidån & ‘next to; beside’ \\%pit 041324.12m03s
sinne / sin & ‘inside’ \\
sissa / sis & ‘into’ \\
siste & ‘out of’ \\
tjadá	& ‘throughout’ \\%cf. pit0906\_Ahkajavvre\_b.047
vuolen & ‘under’ \\
vusste & ‘against’ \\%cf. pit110413a.085/.087/.089
åvdon & ‘in front of’ \\
åvdost & ‘for’ \\
\hline
\end{tabular}%}
...
Postpositions are complemented by NPs in the genitive case. %UM’s suggestion to replace the following sentences with this previous one
Two examples are provided in \REF{postpositionEx1} and \REF{postpositionEx2}; for more on the syntactic behavior of postpositions in postpositional phrases as well as more examples, see Section \ref{postpositionalPhrases}. %on postpositional phrases, including examples from the corpus illustrating postpositions.%UM: delete
...
\glll	ja dä skåvlåmáná minnin dan bajel sparkijin\\
	...
	and then school-child\BS\Sc{nom.pl} go-\Sc{3pl.pst} \Sc{dem}-\Sc{dist}-\Sc{gen.sg} over kick\_sled-\Sc{com.sg}\\\nopagebreak
\Transl{and then the school children crossed that by kick-sled}{}	\Corpus{090915}{031}
\z

\subsection{Prepositions}\label{prepositions}\index{prepositions}\index{parts of speech!prepositions}
%UM: suggests:
With the exception of \It{dugu} ‘like’, which governs a noun in either the essive or the nominative case,\footnote{Cf. Section \ref{essive}.} 
...
\glll	ja badde, åsto badde birra danne\\
	...
	and ribbon\BS\Sc{nom.sg} buy-\Sc{nmlz1}\BS\Sc{nom.sg} ribbom\BS\Sc{nom.sg} around \Sc{dem}-\Sc{dist}-\Sc{gen.sg}\\\nopagebreak
\TranslLong{and ribbon, purchased ribbon around that}{}	\CorpusLink{080708_Session08}{080708\_Session08}{012}
\z
...
However, the complement \It{nällje kronor} ‘four crowns’ (referring to the Swedish currency) consists of the \PS\ numeral \It{nällje} and a Swedish borrowing \It{kronor} which is inflected according to Swedish grammar (\It{kron-or} ‘crown-\PLs’), and not \PS\ grammar, so it is impossible to know with this data which case \It{badjel} governs as a preposition. 
\ea\label{prepositionEx2}%
\glll	så åtjojmä badjel nällje kronor tjilos dalloj\\
	så åtjo-jmä badjel nällje kronor tjilos dalloj\\
	so receive-\Sc{1pl.pst} over four crowns kilogram at\_that\_time\\\nopagebreak
\TranslLong{so we received more than four Swedish crowns per kilogram back then}{}	\CorpusLink{0906_Ahkajavvre_a}{0906\_Ahkajavvre\_a}{159}
\z
...
\Bf{Conjunctions} in \PS\ form a closed class that connect phrases or clauses. A list of some conjunctions and their English translation equivalents can be found in Table \vref{conjunctionTable}. Note that the conjunctions \It{eller}, \It{men} and \It{att} are borrowings from Swedish. 
\begin{table}\centering%
...
jala			&\CH		&\CH		& ‘or’	\\
jus			&			&\CH		& ‘if’	\\
maŋŋel		&			&\CH		& ‘after’	\\%JW: conjunction??
men			&			&\CH		& ‘but’	\\
vaj			&?			&\CH		& ‘or’	\\
...
åvdål		&			&\CH		& ‘before’	\\%JW: conjunction??
\hline
\end{tabular}
\end{table}
...
In \REF{conjunctionEx3} and \REF{conjunctionEx4} \It{jala} ‘or’ connects numeral-APs and NPs, respectively. %Note that, in \REF{conjunctionEx4}, \It{jala} connects two nominal NPs, unlike the English translation, in which two prepositional phrases are connected. %UM: not necessary
...
The conjuction \It{jala} ‘or’ can also connect verbs, as in \REF{conjunctionEx5}. 
...
An \Bf{interjection} is an individual word that is syntactically an utterance of its own (syntactically at the same level as entire clauses). As such, interjections are not a part of another clause. Interjections often indicate a speaker’s feelings or attitude towards an event. The data in the corpus is quite limited, and it is beyond the scope of the current study to describe interjections in detail, so the 
list of interjections %, two of which are used as curse words, 
and their English translation equivalents provided in Table \vref{interjectionTable} is preliminary and subject to amendment pending future research. %A list of interjections %, two of which are used as curse words, 
...
\glll	\Tn{A:} nå, guste dån bådá?\\
	{} nå guste dån bådá\\
	{} well from\_where \Sc{2sg.nom} come\BS\Sc{2sg.prs}\\\nopagebreak
...
\glll	\Tn{A:} ja dä muv biena Rahka\\
	{} ja dä muv biena Rahka\\
	{} and then \Sc{1sg.gen} dog\BS\Sc{nom.sg} Rahka\\\nopagebreak
\TranslMulti{and then my dog Rahka}{}\\
\glll	\Tn{B:} nå, duv biena aj\\
	{} nå duv biena aj\\
...
\TranslLongMulti{so then we were able to, Josh and I were able to ski up to this\\\nopagebreak\hspace{10pt} here Big Point}{}\\	%\Corpus{100404.360}
\glll	\Tn{B:} nå.\\
	{} nå\\
	{} well\\\nopagebreak
...
The interjection \It{jå} ‘yes’, also a Swedish borrowing,\footnote{< Sv. \It{jå}, a common pronunciation of \It{ja} ‘yes’.} 
...
\glll	\Tn{A:} ‘tjåddjåk’, nä, ij lä ‘tjåddjåk’\\
	...
	{} esophagus\BS\Sc{nom.sg} no \Sc{neg}\BS\Sc{3sg.prs} be\BS\Sc{conneg} esophogus\BS\Sc{nom.sg}\\\nopagebreak
\TranslMulti{‘tjåddjåk’, no, it’s not ‘tjåddjåk’}{}\\
\glll	\Tn{B:} jå, lä tjåddjåk\\
	{} jå lä tjåddjåk\\
	{} yes be\BS\Sc{3sg.prs} esophogus\BS\Sc{nom.sg}\\\nopagebreak
