...
At least one derivational affix seems to exist which derives an adverb from an adjective: the suffix \It{-git}, as illustrated by Table \vref{derivedADVsTable}. 
...
A list of some lexical adverbs is provided in Table \vref{lexicalAdverbTable}.\footnote{The adverbs \It{ber}\TILDE\It{bar}, \It{kan} and \It{så} are Swedish loans; cf. Sw. \It{bara} ‘only’, \It{kanske} ‘maybe’ and \It{så} ‘so’.} 
...
Conjunctions in \PS\ form a closed class of words that connect phrases or clauses. A list of some conjunctions and their English translation equivalents can be found in Table \vref{conjunctionTable}. Note that the conjunctions \It{att}, \It{eller} and \It{men} are borrowings from Swedish. 
\begin{table}\centering%
...
Finally, in \REF{conjunctionEx6}, the loan conjunction \It{eller} ‘or’\footnote{< Sw. \It{eller} ‘or’.} connects NPs. 
