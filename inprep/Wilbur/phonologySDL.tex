...
Table \vref{syllTempExs} provides some examples for the syllabic structure\is{syllable structure} of words with up to five syllables. 
\begin{table}[h]\centering
...
Due to systematic restrictions on the distribution of a number of segments and consonant clusters as well as to the prosodic domains of morphophonological\is{morphophonology} processes, it is useful to name and describe various prosodic positions for polysyllabic words. The domains themselves are described below, while the relevant phonological restrictions and morphophonological processes are described in the pertinent sections on consonant phonemes (\SEC\ref{consonants}), vowel phonemes (\SEC\ref{vowelPhonemes}) and morphophonology (\SEC\ref{morphophonology}). Only a very limited number of recent loan words do not adhere to this structure. 
...
\subsubsection{Foot}\label{foot}\is{foot}
...
\subsection{Intonation in utterances}\label{utteranceIntonation}\is{intonation}
...
For plosives\is{plosives} and affricates\is{affricate}, only one stop closure is formed, and the overall duration of the stop closure is longer than the duration of a corresponding single consonant. Some examples are provided in \REF{outside} through \REF{motherNOMSG}.
...
\subsubsection{Plosives}\label{Plosives}\is{plosives}%%%%%%%% PLOSIVES %%%%%%%%%%%%%%%
...
\paragraph{Dialect variation and the historical voiced dental fricative *ð}\label{ethVariation}\is{dialect variation}%/r\TILDE d\TILDE ð/
...
The segments /mː\,nː\,ɲː\,ŋː/ are geminate\is{geminate} bilabial, alveolar, palatal and velar (respectively) nasal consonant phonemes. 
...
The segment /jː/ is a geminate\is{geminate} central approximant. It only occurs in the consonant center, and becomes devoiced\is{devoicing} [jj̥ː] towards the end of its realization when preceding a preaspirated\is{preaspiration} phoneme.  
