...
\begin{tabular}{c c c  l }
			&\MC{2}{c}{\It{examples}}&	\\
\It{template}	& \It{IPA}	& \It{orth}		& \It{gloss} \\\hline
{VC}	&aj		&\It{aj}		& ‘also’ \\
...
{CVCCC}& taːjst	&\It{dájst}		& ‘from these’ (\Sc{dem}-\Sc{prox}-\Sc{elat.pl}) \\\hline
\end{tabular}
\end{table}
\FB
...
\begin{tabular}{ l c |c c c| c c c l }
		&\MC{7}{c}{\It{p\,r\,o\,s\,o\,d\,i\,c\hspace{3em}d\,o\,m\,a\,i\,n\,s}}							&\\\cline{3-5}
%		&\MC{7}{c}{\It{prosodic domains}}							&\\\cline{3-5}
%		&\MC{1}{c}{\It{foot}}	&\MC{3}{c}{}&			&	&	&\\\cline{3-5}
\It{IPA}	&\It{foot\,onset}&\It{V1}&\It{C-center}&\It{V2}&\It{C2} &\It{V3}&\It{C3}&\It{gloss} \\\hline
...
\hline
		&	&\MC{3}{c|}{\It{minimal core}}&			&	&	&\\\cline{3-5}
\end{tabular}}
\end{table}
...
\begin{tabular}{lll}
\Sc{nom.sg}	&\Sc{acc.sg}		&\It{gloss}	\\\hline
 sálbma		& sálmav		& ‘psalm’	\\
 girrje		& girjev		& ‘book’	\\
...
\begin{tabular}{c c c }
\It{preceding segment}	& \It{realization of preaspiration}	&\It{example} \\\hline
...
{C\sub{1}}	&& \MC{2}{c}{C\sub{2}}	&\It{attested CCs}\\\hline
{plosive} &\PLUS& \MC{2}{c}{sonorant}		& pr, pl, tr, kn, kr, kl \\%\hline%\cline{3-4}%6
{sibilant} &\PLUS& \MC{2}{c}{obstruent}	& sp, st, sk, sv, ʃk, ʃv	\\%\hline%\cline{3-4}%6
{fricative} &\PLUS& \MC{2}{c}{sonorant}	& fr, fl, sm, sn, sɲ, sl, ʃm, ʃɲ, ʃl \\\hline%9 Total: 21
\end{tabular}%\caption{}\label{}
\end{table}
\begin{table}\centering
\caption{Tripartite consonant clusters in syllable onset position}\label{syllableOnsetCCCsB}
\begin{tabular}{c c c c c l}
C\sub{1}	&& C\sub{2} && C\sub{3}	&\It{attested CCCs}\\\hline
...
%\resizebox{\columnwidth}{!}{%
\begin{tabular}{c c c l }
\MC{1}{c}{C\sub{1}}			&& C\sub{2}&\It{attested CCs}\\\hline
...
\MC{1}{c}{C\sub{1}}			& C\sub{2}			&\It{possible clusters}\\\hline
...
are fairly common, and correspond paradigmatically to CCs lacking the plosive but with only the fricative and sonorant (see above on heterosyllabic CCs and §\,\ref{Cgrad} on consonant gradation). The other three tripartite CCs /jst mst rtm/ are only attested in one or two words each,\footnote{Recent loan words from Swedish may also contain tripartite CCs not found elsewhere in \PS, e.g.: \It{kɔ\Bf{nst}ɔ} ‘art’ <Swedish \It{konst}.} and there is not enough data to reach any further conclusions at this point.
\begin{table}\centering
\caption{Tripartite consonant clusters in the consonant center}\label{CCenterCCCs}
\resizebox{1\linewidth}{!} {
\begin{tabular}{ c c c c c  p{201pt} }
\MC{1}{c}{C\sub{1}}	&&\MC{1}{c}{C\sub{2}}&& C\sub{3}	&\It{possible CCCs}\\\hline
...
rm	&:& rpm	&&	lŋ	&:& lkŋ \\
rn	&:& rtn	&&	jm	&:& jpm \\
rŋ	&:& rkŋ 	&&	jn	&:& jtn \\\cline{1-3}\cline{5-7}%
\end{tabular}
\end{table}
...
\It{vowel} &\It{V1}	&\It{V2}	&\It{V3}	\\\hline
i	&\PLUS	&\PLUS	&\PLUS	\\
e	&\PLUS	&\PLUS	&\MINUS	\\
ɛ	&\PLUS	&(\MINUS)&\PLUS	\\
a	&\PLUS	&\PLUS	&\PLUS	\\
...
\It{phonemic}&\MC{2}{c}{\It{phonetic}}&\It{orthography}&\It{gloss} \\\hline
