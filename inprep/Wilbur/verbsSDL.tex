...
\end{tabular}}
\end{table}
Note that the vowel in V2 position (\It{a}, \It{e} and \It{i}) in all forms is the inflectional class marker for Class III verbs (cf. Section \ref{VclassIII}); thus the stem has five allomorphs: \It{buälld-}, \It{bualld-}, \It{buold-}, \It{bulld-} and \It{buld-}.\footnote{The examples used in this description of non-linear verb morphology is based on the current \PS\ orthography, which is still a work in progress. Because the orthography is to a great extent phonemic, orthographic representations are sufficient for the current discussion.} 
...
xyz	&:& xz	&/ʧaːjpma/	&:&/ʧaːjma/	& ‘laugh’\\%\hline%
...
\marginpar{check if -tj blocks CG/umlaut in verbs!}Furthermore, some derivational suffixes (such as the diminutive suffix \It{-tj}) can block consonant gradation and umlaut from happening in the new derived form. 
