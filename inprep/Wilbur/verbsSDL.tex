...
Verbs in Pite Saami form an open class of words which are defined syntactically by their ability to head a verb complex\is{phrase types!verb complex}, as well as morphologically by inflecting for person, number, tense and mood. 
...
The progressive\is{aspect!progressive} form of a verb can also be used in an adverbial\is{phrase types!adverbial phrase} function. For instance, \It{tjájbmamin} ‘laughing’ in \REF{ADVverbsEx1} and \It{gullamin} ‘listening‘ in \REF{ADVverbsEx2} are each used as a modal adverbial to indicate a simultaneous activity. 
...
As with any finite verb, the negation verb agrees in person\is{person} and number\is{number} with the subject of the sentence and inflects for tense\is{tense} or mood\is{mood}. The complement verb occurs in a special non-finite verb form called the connegative\is{non-finite verb form!connegative} (glossed as \mbox{\CONNEGs}), which is in the weak grade (when gradation is relevant) and otherwise lacks any additional morphological marking. Examples for present and past indicative as well as imperative forms are provided in \REF{connegEx1} through \REF{negImpEx1}. 
\ea\label{connegEx1}%
