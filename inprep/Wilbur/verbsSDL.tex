...
Verbs in Pite Saami form an open class of words which are defined syntactically by their ability to head a verbal complex\is{verbal complex}, as well as morphologically by inflecting for person, number, tense and mood. 
...
All finite verbs agree in number\is{number} with the subject of the clause and inflect\is{inflection} for singular, dual or plural. 
...
\z
Note that there are a few examples in the corpus in which speakers do not consistently inflect for dual\is{dual}, but instead use the corresponding plural form. 
...
\subsection{Future}\label{futureTense}\is{future}
...
The progressive\is{aspect!progressive} form of a verb can also be used in an adverbial\is{adverbial phrase} function. For instance, \It{tjájbmamin} ‘laughing’ in \REF{ADVverbsEx1} and \It{gullamin} ‘listening‘ in \REF{ADVverbsEx2} are each used as a modal adverbial to indicate a simultaneous activity. 
...
As with any finite verb, the negation verb agrees in person\is{person} and number\is{number} with the subject of the sentence and inflects for tense\is{tense} or mood\is{mood}. The complement verb occurs in a special non-finite verb form called the connegative\is{connegative} (glossed as \mbox{\CONNEGs}), which is in the weak grade (when gradation is relevant) and otherwise lacks any additional morphological marking. Examples for present and past indicative as well as imperative forms are provided in \REF{connegEx1} through \REF{negImpEx1}. 
\ea\label{connegEx1}%
...
\section{Morphological marking strategies on verbs}\label{markingVerbs}\is{inflection}
...
The portmanteau\is{portmanteau} suffixes expressing agreement in person\is{person} and number\is{number} as well as tense\is{tense} or mood\is{mood} in finite verbs are listed in Table \vref{verbSuffixes}. 
...
\subsubsection{Verbal suffixes and syncretism}\label{verbalSuffixesSyncretism}\is{suffix!verbal}
...
In addition to using the inflectional suffixes described above, inflectional\is{inflection} categories for verbs can be marked %for tense, mood, aspect, person and number, as well as for non-finite forms, 
...
\subsection{The potential mood: inflection or derivation?}\label{POTinflection}\is{mood!potential}
...
\section{Inflectional classes for verbs}\label{verbInflectionalClasses}\is{inflection}
...
\item{the number of syllables in the infinitive form\is{syllabic structure}}
...
\begin{itemize}
\item{a bisyllabic infinitive form\is{syllabic structure}}
...
\begin{itemize}
\item{a trisyllabic infinitive form with the class marking suffix \It{-i}\is{syllabic structure}}
