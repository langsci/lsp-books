...
In describing \PS\ clauses, it is useful to begin with basic clauses that contain a full predicate and its arguments, complements and/or adjuncts, before moving on to describing complex clauses which consist of more than one clause linked through coordination or subordination. 
...
\PS\ is an accusative language because %the subject and only nominal argument %UM: delete; cf. Comrie 1989
the only argument of an intransitive verb (S) is marked in the same way as %the subject and %UM: delete; cf. Comrie 1989
the most-agent-like argument of a transitive verb (A): by the nominative case. The %object and %UM: delete; cf. Comrie 1989
most-patient-like argument of a transitive verb (P) is marked differently: by the accusative case. This is illustrated by the following examples, with an intransitive verb in \REF{intrans1} and a (mono-)transitive verb in \REF{monotrans1}.
\ea\label{intrans1}
...
\glll	sågijd mån anav\\
	sågi-jd mån ana-v\\
	birch-\Sc{acc.pl} \Sc{1sg.nom} have-\Sc{1sg.prs}\\\nopagebreak
...
To illustrate this syntactic flexibility, examples of SOV and OSV %and VS %removed due to comment from UM about Adv+V+S - no ex. with V in first w/o dä or some other adv! (with overt subject and only one verb)
...
\korr{030}information provided by context alone are frequently not realized overtly, as in \REF{deviantConstituentOrder4} above and \REF{missingNP1} through \REF{missingNP4} below.
\ea\label{missingNP1}
...
in \REF{missingNP5},\footnote{The source of the example in \REF{missingNP5} is the recording ‘\hyperlink{pit05HilpertDialog}{pit05HilpertDialog}’, which was collected by linguist Martin Hilpert during a pilot project he completed on \PS\ in 2005. I am very grateful to him for providing me with his recordings and annotations. Note that Martin’s recordings are not included in the \PS\ documentation corpus.} 
which consists only of nothing more than the copula verb inflected for \Sc{3sg.prs}.
\ea\label{missingNP5}
\glll	\Tn{A:} ja tjáppa dállke!\\
	{} ja tjáppa dállke\\
...
The short example text in \REF{mouseText4} below should serve to give an impression of how information structure may be the driving force behind constituent ordering at clause-level. Here, the speaker is talking about looking inside her mother’s shoes after discovering that a mouse had been in them.
\ea\label{mouseText4}%
\glll	\Tn{A:} ja danne vuojdniv unna jåŋåtjav.\\
	{} ja danne vuojdni-v unna jåŋå-tja-v\\
	{} and there see-\Sc{1sg.pst} small\BS\Sc{attr} lingonberry-\Sc{dim}-\Sc{acc.sg}\\\nopagebreak
\TranslMulti{and there I saw a little lingonberry}{}\\
%\ex\label{mouseText5}%
\glll	\Tn{B:} jahkav skafferijav lä danne adnam,\\
	{} jahka-v skafferija-v lä danne adna-m\\
	{} believe-\Sc{1sg.prs} pantry-\Sc{acc.sg} be\BS\Sc{3sg.prs} there have-\Sc{prf}\\\nopagebreak
\TranslMulti{I think (the mouse) had a pantry there}{}	\Corpus{100404}{353-354}
\z

In \REF{mouseText4}{a} the topic is \It{danne} ‘there’, which refers to the shoes (the topic of the anecdote at this point) and is clause-initial. The constituent \It{jåŋåtjav} ‘lingonberry’ is the focus, but it is not particularly significant in the anecdote, and it follows the finite verb \It{vuojdniv} ‘I saw’. 
However, when particular emphasis is placed on the focus, as in the following clause in \REF{mouseText4}{b}, the constituent in focus can be fronted. Here, \It{skafferijav} ‘pantry’ is in focus, and receives particular emphasis\footnote{The NP \It{skafferijav} ‘pantry’ probably receives special emphasis because it personifies the behavior of the mouse in a light-hearted way by claiming that a mouse can have a pantry.} by occurring before the verbal complex \It{lä danne adnam} ‘has had there’, while the topic (the mouse) is not realized overtly at all, but implied by the context and by the finite verb form inflected for \Sc{3sg}. 
...
NPs referring to \korr{031}information provided by context alone are not necessarily realized overtly; as a result, the syntactic subject and other verbal arguments are often not overtly present. 

The following sections first present basic declarative clauses with intransitive and transitive verbs, existential clauses, copula clauses and complex verbal constructions consisting of more than one verb (Section \ref{declClauses}). Section \ref{interrogClauses} then deals with interrogative clauses, %highlighting the differences between interrogative and declarative clauses, %Negation, which is in fact a special case of complex verbal construction, is treated in Section \ref{},
...
\Transl{I laugh}{} \Corpus{100323a}{005}
\z
\ea\label{basicIntransDeclaratives3}
\glll	dáj Skaile ello såkoj\\
	d-á-j Skaile ello såko-j\\
	\Sc{dem}-\Sc{prox}-\Sc{gen.pl} Skaile\BS\Sc{gen.pl} reindeer\_herd\BS\Sc{nom.sg} drown-\Sc{3sg.pst} \\\nopagebreak
\TranslLong{The Skaile family’s reindeer herd drowned}{} \CorpusLink{0906_Ahkajavvre_b}{0906\_Ahkajavvre\_b}{002}
\z


\subsubsection{Clauses with a passive verb}\label{passiveVoice}\index{passive}\index{syntax!passive voice}
When a transitive verb is passivized,\footnote{Transitive verbs can be passivized using the derivational suffix \It{-duvv}; cf. Section \ref{VdervPassives} on derivational morphology and Section \ref{passiveVinflection} on inflectional morphology for passives.} 
its valency is reduced, and it becomes intransitive. In this, the patient is the subject of the verbal complex, and therefore marked by nominative case; the agent may be left out. This is illustrated by the pair of elicited examples in \REF{passVoiceEx1} and \REF{passVoiceEx2}, and the example from a narrative in \REF{passVoiceEx2b}. %The first clause is a transitive verb in the active, while the second 
...
\glll	almatj bedja virbmijd ehket\\
	...
	person\BS\Sc{nom.sg} put\BS\Sc{3sg.prs} fishing\_net-\Sc{acc.pl} evening\\%\BS\Sc{nom.sg}\\\nopagebreak
...
\z
\footnotetext{A \It{tjårvebielle} (lit.: horn-half) is the \PS\ term used to describe a reindeer with only one antler remaining after the other antler has broken off.} 

The subject frequently follows the verb because the subject is often the focus of the clause, but as the clause in \REF{existential3} shows, the subject may occur before the verb if it is the topic and/or presupposed knowledge.\footnote{Cf. Section \ref{infoStructure} on information structure.}
\ea\label{existential3}
\glll	…ja motora vadnasij. ‘motora’ ij gävdnu sáme gielan\\
	ja motora vadnasi-j motora ij gävdnu sáme giela-n\\
	and motor\BS\Sc{gen.sg} boat-\Sc{com.pl} ‘motor’ \Sc{neg}\BS\Sc{3sg.prs} exist\BS\Sc{conneg} Saami\BS\Sc{gen.sg} language-\Sc{iness.sg}\\\nopagebreak
\TranslLong{…and with motor boats. There is no (word for) “motor” in the Saami language}{} \Corpus{080924}{482,484}
\z


\subsection{Copula clauses}\label{copulaClauses}
There are several types of copula clauses in Pite Saami. All of these feature the copula verb \It{årrot} ‘be’, which inflects for number and person of the subject and is marked for tense, aspect and/or mood (see Section \ref{theCopulaVerb} for more details). 
...
\glll	Mattijá lij morbror munje\\
	...
	Matthias\BS\Sc{nom.sg} be-\Sc{3sg.pst} uncle\BS\Sc{nom.sg} \Sc{1sg.ill}\\\nopagebreak
...
\z
\footnotetext{Note that \It{morbror} ‘maternal uncle’ is a nonce borrowing from Swedish.}
...
\korr{033}A copula clause can also function as an existential clause when it includes a temporal adjunct. In such cases, the existence of the subject referent is posited at that particular time indicated by the adjunct. Pragmatically, this usually announces an event connected to the subject referent. Typically, the temporal referent occurs first in the sentence, then the copula verb, and the subject is last (just as with the existential verb \It{gädvnut}; cf. Section \ref{existentialClauses}), as it is usually the focus. 
This is illustrated by the example in \REF{copulaNr}.
\ea\label{copulaNr}
\glll	ja dále'l káffa\\
	ja dále=l káffa\\
	and now=be\BS\Sc{3sg.prs} coffee-\Sc{nom.sg}\\\nopagebreak
...
In the corpus, such possessive constructions always have this constituent order 
...
it is very uncommon in the \PS\ corpus, and almost exclusively limited to elicitation sessions. The elicitation scenario may have had an effect on the constituent order,\footnote{The equivalent Swedish structure is also normally \Sc{possessor \PLUS\ verb \PLUS\ possessed}.} 
...
expressing possession, as in \REF{copula7}, is now the standard in Pite Saami.\korr{051}\footnote{While this construction using the transitive verb \It{adnet} ‘have’ clearly is not a copula clause, it is worth pointing this out here, particularly for any readers from Uralic studies, because the copular construction is no longer the true standard for expressing possession at clause level.}
\ea\label{copula7}
\glll	ja dä inijmä gusajt \\
	ja dä ini-jmä gusa-jt \\
...
\glll	mij máhtep ságastit Bidumsáme gielav\\
	mij máhte-p ságasti-t Bidum-sáme giela-v\\
...
is the complement to the modal verb, as in the interrogative clause in \REF{modalVerbs5}.
...
\z
\ea\label{future2}%
\glll	jo, da lä akta vuoberis,\footnotemark {} gallga giesset ulgus\\
	jo da lä akta vuoberis gallga giesse-t ulgus\\
...
\glll	jus galga sáme viesuv valldet, dä galga mielagav dal navt rutastit\\
	jus galga sáme viesu-v vallde-t dä galga mielaga-v dal navt rutasti-t\\
	if will\BS\Sc{3sg.prs} Saami\BS\Sc{gen.sg} life?-\Sc{acc.sg} take-\Sc{inf} then should\BS\Sc{3sg.prs} sternum-\Sc{acc.sg} then so pull-\Sc{inf}\\\nopagebreak
\Transl{if I did this in the Saami way, then I would pull the sternum like this}{} \Corpus{080909}{097}
\z
...
\glll	denne liv riegadam\\
	...
	there be-\Sc{1sg.prs} be\_born-\Sc{prf}\\\nopagebreak
\Transl{I was born there}{} \Corpus{090702}{008}
\z
\ea\label{perfClause2}%maybe move this NEG example to the NEG section?!
\glll	lä dån mannam nagin bále ja tjuvvum Vistekij?\\
	lä dån manna-m nagin bále ja tjuvvu-m Vistek-ij\\
...
\Transl{but they didn’t eat from this}{} \CorpusLink{080708_Session03}{080708\_Session03}{019}
\z
\ea\label{negation3}%
\glll	ittjij åbbå gävdno vuodja åsstet\\
	ittji-j åbbå gävdno vuodja åsste-t\\
	\Sc{neg-3sg.pst} at\_all exist\BS\Sc{conneg} butter\BS\Sc{nom.sg} buy-\Sc{inf}\\\nopagebreak
\Transl{there wasn’t any butter to buy}{} \CorpusLink{080708_Session03}{080708\_Session03}{006}
\z
...
\glll	dä iv lä åbbå gullam dav\\
	...
	\Sc{ptcl} \Sc{neg}-\Sc{1sg.prs} be\BS\Sc{conneg} at\_all hear-\Sc{prf} \Sc{dem}-\Sc{dist}-\Sc{acc.sg}\\\nopagebreak
...
While \PS\ constituent order is generally flexible,\footnote{Cf. Section \ref{constituentOrderClauses}.} there are no examples in \PSDP\ corpus of the verb of negation occurring after the negated complement, but instead the connegative complement verb always follows the finite negation verb in a clause. 

\section{Interrogative clauses}\label{interrogClauses}\index{clause types!interrogative}
Constituent interrogative clauses in \PS\ are consistently marked as such syntactically, and thus are distinct from declarative clauses. Polar interrogative clauses, on the other hand, do not consistently differ from declarative clauses, although some syntactic constructions are more common than others. The following sections deal first with constituent interrogative clauses, then polar interrogative clauses in more detail.

\subsection{Constituent interrogative clauses}\label{constituentQs}
Constituent interrogative clauses are the only type of independent clause in \PS\ which is consistently marked syntactically as a clause type. Specifically, every constituent interrogative clause is marked as such by having an interrogative word in clause-initial position. This interrogative word can be an interrogative pronoun, which, as with any pronoun, inflects for case and number consistent with its grammatical role in the clause, while the humanness of its (expected) referent determines the choice of the root.\footnote{Interrogative pronouns referring to non-human NPs all feature a \It{ma-} root, while those referring to human NPs have a \It{ge-} root. Interrogative adverbs also begin with \It{g-}. Cf. Section \ref{interrogativePronouns}.} 
Some examples can be found in \REF{questionWordQ1} through \REF{questionWordQ3}. 
...
\glll	guste dån bådá\\
	guste dån bådá\\
	where\_from \Sc{2sg.nom} come\BS\Sc{2sg.prs}\\\nopagebreak
...
\Transl{where did you all pick them?}{} \Corpus{080924}{168}
\z
...
As with any \PS\ clause, the syntactic subject does not have to be realized overtly. In such cases, the finite verb is still usually word initial, as in \REF{polarQinversion4}. 
...
\glll	juhkum lä gajtsa mielkiev?\\
	juhku-m lä gajtsa mielkie-v\\
	drink-\Sc{pft} be\BS\Sc{2sg.prs} goat\BS\Sc{gen.sg} milk-\Sc{acc.sg}\\\nopagebreak
...
\glll	bårråm lä dån biergov danne?\\
	...
	eat-\Sc{pft} be\BS\Sc{2sg.prs} \Sc{2sg.nom} meat-\Sc{acc.sg} there\\\nopagebreak
...
on the finite verb; these morphemes express imperative mood as well as the number of the implied subject of the clause. 
The finite verb tends to be in clause-initial position, as shown by the examples in \REF{impClause1} and \REF{impClause2}.
\ea\label{impClause1}%
\glll	giehto naginav dan Luodaure birra\\
	giehto nagina-v d-a-n {Luodaure\footnotemark} birra\\
	tell\BS\Sc{sg.imp} something-\Sc{acc.sg} \Sc{dem}-\Sc{dist}-\Sc{gen.sg} Luodaure\BS\Sc{gen.sg} about\\\nopagebreak
\Transl{say something about this Luodaure}{} \Corpus{080924}{314}
\z
...
\z
\footnotetext{\It{Luodaure} is the name of a lake.}
...
\z
\noindent However, no other examples for such constructions are found in the corpus, and this constituent ordering is likely due to this phrase being a common and non-productive lexicalized expression calqued from the Swedish expression \It{tack ska du ha!} (literally ‘thanks you shall have!’). 
...
\glll	jus sån vuosjatja káfav\\
	...
	if \Sc{3sg.nom} prepare\_coffee-\Sc{pot}-\Sc{3sg} coffee-\Sc{acc.sg}\\\nopagebreak
\Transl{if he will perhaps make some coffee}{}	\CorpusE{110404}{269}
\z
...
As mentioned in Section \ref{POTmood}, the potential mood can also be used as a less severe command. This resembles clauses in the imperative voice, which never occur with an overt subject. In the corpus, tokens of such commands marked by the potential mood never occur with an overt subject, as shown %\marginpar{\REF{potSyntaxEx4} already in Section on \POTs!} 
in example \REF{potSyntaxEx4}. 
\ea\label{potSyntaxEx4}%
\glll	vuosjatja káfav\\
	vuosja-tj-a káfa-v\\
	prepare\_coffee-\Sc{pot}-\Sc{2sg} coffee-\Sc{acc.sg}\\\nopagebreak
...
\glll	men ijtjin del bårå dan siste, men ednen biebmojt biergojt ja dále návte deggara sinne\\
	...
	but \Sc{neg}-\Sc{3pl.pst} obviously eat\BS\Sc{conneg} \Sc{dem}-\Sc{dist}-\Sc{gen.sg} out but have-\Sc{3pl.pst} food-\Sc{acc.pl} meat-\Sc{acc.pl} and now like\_this such\BS\Sc{gen.pl} in\\\nopagebreak
\TranslLong{but they obviously didn't eat from this, but they had food, meat and so on in such things}{} \CorpusLink{080708_Session03}{080708\_Session03}{019}
\z
...
A \PS\ clause can be subordinate to another clause or to a nominal phrase. %are normally marked as being subordinate to a matrix phrase. 
...
\It{predicate type}	&\It{subordination strategy}	\\\dline%&\it  typical verbs \\\dline
\MR{2}{*}{finite}	& complementizer \It{att}	\\\cline{2-2}%\hline%& hållåt, diehtet \\\hline
%finite				& fronted question word/pronoun	\\\cline{2-2}%\hline%& diehtet \\\hline
				& juxtaposition	\\\hline%question/relative pronoun	& finite		\\\hline%& skenet \\\hline
...
First, the borrowed complementizer \It{att}\footnote{Cf. the Swedish particle \It{att}, which is, among other things, also a complementizer.} 
...
\glll	men mån tuhtjiv dat lij nav suohtas tieltajn viessot\\
	...
	but \Sc{1sg.nom} think-\Sc{1sg.prs} \Sc{dem}-\Sc{dist}-\Sc{nom.sg} be\BS\Sc{3sg.pst} so nice-\Sc{sg} tent-\Sc{iness.pl} stay-\Sc{inf}\\\nopagebreak
...
are accompanied by complement clauses headed by an infinitive verb, %\marginpar{too many examples? delete one or two?}%JW: UM says ‘no, leave them’
as in \REF{complClauseJuxInf1} through \REF{complClauseJuxInf4}.
...
\glll	ja dat lij samma, jus del lij gualpana vaj biodaka vaj smav biedtsastak\\
	...
	and \Sc{dem}-\Sc{dist}-\Sc{nom.sg} be-\Sc{3sg.pst} same if then be-\Sc{3sg.pst} flat\_pine\_heath\BS\Sc{nom.sg} or high\_ground\BS\Sc{nom.sg} or small\BS\Sc{attr} pine\_forest\BS\Sc{nom.sg}\\\nopagebreak
\TranslLong{and that was the same whether it was flat-pine-heath or higher-ground or small pine-forests}{} \Corpus{100405a}{009}
\z
...
\TranslLong{If you slaughtered them correctly, then you would take them out before hanging (them) up}{} \Corpus{080909}{105}
\z
...
With this in mind, it is certainly plausible that \PS\ can make use of non-finite verb forms other than those mentioned above. However, there is exceptionally little evidence of such forms in the present corpus, and even this is limited to progressive forms in elicitation sessions. For instance, as mentioned in Section \ref{ADVverbs}, the progressive non-finite verb form can be used adverbially. One example featuring the progressive form \It{gullamin} ‘listening’ is repeated here in \REF{ADVverbsEx2repeat}. Even here, it is not clear whether such non-finite forms can include arguments or adjuncts. 
...
\TranslLong{those are times which have been}{(i.e.: ‘those were the good old days’)} \Corpus{090702}{409}
\z
...
\Transl{they were young people who were watching (the herd)}{} \CorpusLink{0906_Ahkajavvre_b}{0906\_Ahkajavvre\_b}{017}
\z
...
\glll	dat lä náhppe man sisa båhtjen buhtsujt\\
	...
	\Sc{dem}-\Sc{dist}-\Sc{nom.sg} be-\Sc{3sg.prs} milking\_bowl\BS\Sc{nom.sg} \Sc{rel}-\Sc{gen.sg} into milk-\Sc{3pl.pst} reindeer-\Sc{acc.pl}\\\nopagebreak
...
\TranslLong{but here was the pond whose name was, which was the name of that, this, our Arjeplog}{} \Corpus{090915}{013}
\z
