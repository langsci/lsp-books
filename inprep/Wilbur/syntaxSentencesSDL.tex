...
the only argument of an intransitive\is{intransitive} verb (S) is marked in the same way as the most-agent-like argument of a transitive\is{transitive} verb (A): by the nominative\is{case!nominative} case. 
...
The direct object of a ditransitive\is{ditransitive} verb is also in the \is{case!accusative}accusative case, while the indirect object %’goal’ is semantic role, not grammatical relation which is needed here
...
Attempting to determine constituent order patterns is further complicated by the fact that it is sometimes impossible to tell what the constituent ordering is because NPs\is{phrase types!nominal phrase} referring to %presupposed 
...
in \REF{missingNP5}, which consists of nothing more than the copular verb\is{copular verb} inflected for \Sc{3sg.prs}.
\ea\label{missingNP5}
...
\subsection{Copular clauses}\label{copulaClauses}
There are several types of copular clauses in Pite Saami. All of these feature the copular verb\is{copular verb} \It{årrot} ‘be’ %, which inflects for number and person of the subject and is marked for tense or mood when 
...
When the complement of the copula is an NP\is{phrase types!nominal phrase} in nominative\is{case!nominative} case, it identifies or classifies the subject referent, as in \REF{copula1} and \REF{copula2}, respectively.
\ea\label{copula1}
...
The complement of a copular clause can be one or more \is{phrase types!adjectival phrase}adjectival phrases headed by a predicative adjective which ascribes properties to the subject referent, as in \REF{copula3}.
\ea\label{copula3}
...
\z
However, when the subject of the modal verb \It{sihtat} is not coreferential with the subject of the complementing verb complex\is{phrase types!verb complex}, then a finite verb clause %with a finite verb inflected for the second subject 
...
together with a non-finite complement verb\is{non-finite verb form} is used to mark the perfective and progressive aspects\is{aspect}. This auxiliary verb is homophonous with the copular verb\is{copular verb}, and is also glossed as ‘be’.
...
\z
In the examples above, the non-finite complement to the connegative\is{non-finite verb form!connegative} verb is a lexical verb. In the following examples in \REF{negation4} through \REF{negation6}, the complement connegative verb is a modal or auxiliary verb whose own complement then follows in the appropriate non-finite form.
\ea\label{negation4}%
...
Certain types of \PS\ clauses can be subordinate to another clause or to a nominal phrase\is{phrase types!nominal phrase}. %are normally marked as being subordinate to a matrix phrase. 
...
inflects for case according to the syntactic function it fills within the relative clause, and for the number of the NP\is{phrase types!nominal phrase} that it modifies, as illustrated by \REF{relClause1} through \REF{relClause3}.
\ea\label{relClause1}%
...
\z
This shows that it is possible, in this case, for a relative clause to not be embedded syntactically in the modified NP, as the relative clause can occur outside the postpositional phrase\is{phrase types!postpositional phrase} which the modified NP is a constituent of. It should be emphasized that only a postposition can split a relative clause from the matrix NP it modifies.
