...
\TranslLong{…with motor boats. There is no (word for) “motor” in the Saami language}{} \Corpus{080924}{482,484}
\z
...
\glll	Mattijá lij morbror munje\\
	...
	Matthias\BS\Sc{nom.sg} be-\Sc{3sg.pst} maternal\_uncle\BS\Sc{nom.sg} \Sc{1sg.ill}\\\nopagebreak
...
\z
\footnotetext{Note that \It{morbror} is a nonce borrowing from Swedish. Cf. Sw. \It{morbror} ‘maternal uncle’.}
...
expressing possession, as in \REF{copula7}, is now the standard in Pite Saami.\korr{051}\footnote{While this construction using the transitive verb \It{adnet} ‘have’ clearly is not a copula clause, it is worth pointing this out here, particularly for any readers from Uralic studies, because the copular construction (as in the example in \REF{copula6}) is no longer the true standard for expressing possession at clause level in \PS.}
\ea\label{copula7}
...
is the complement to the modal verb, as in the negated clause in \REF{modalVerbs5}. \korr{081}Note that, here, \It{sihtat} is in the connegative non-finite form \It{sida}. %Note that, Here the complement of the main verb \It{sida} (the connegative non-finite form) is the 
...
\glll	dä iv lä åbbå gullam dav\\
	...
	then \Sc{neg}-\Sc{1sg.prs} be\BS\Sc{conneg} at\_all hear-\Sc{prf} \Sc{dem}-\Sc{dist}-\Sc{acc.sg}\\\nopagebreak
...
\glll	man mällgadav ana dajd riehpenen\\
	...
	how long-\Sc{acc.sg} have\BS\Sc{2sg.prs} \Sc{dem}-\Sc{dist}-\Sc{acc.pl} smoke\_hole-\Sc{iness.sg}\\\nopagebreak
...
As mentioned in Section \ref{POTmood}, the potential mood can also be used as a less severe command. This resembles clauses in the imperative voice by also never occuring with an overt subject, as shown %\marginpar{\REF{potSyntaxEx4} already in Section on \POTs!} 
