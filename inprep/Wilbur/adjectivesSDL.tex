...
Adjectivals in \PS\ are defined syntactically by their ability to head an adjectival phrase\is{phrase types!adverbial phrase} (AP). They can be divided into four sub-categories based on both syntactic and morphological behavior, as summarized in Table \vref{adjectivalsSummary}.
...
Attributive adjectives form the head of an AP modifying the head of the matrix nominal phrase\is{phrase types!nominal phrase}, and are normally not subject to inflectional\is{inflection} morphology. 
...
While attributive adjectives form the head of an attributive AP embedded in an NP\is{phrase types!nominal phrase}, predicative adjectives form the head of an AP which is the complement of the copular verb\is{copular verb} \It{årrot} ‘be’ and ascribe a property to the subject referent. %
...
Predicative adjectives can be used to compare\is{comparative} the referents of nominal phrases. If both referents are considered equal concerning the characteristic of comparison, then the NP of comparison is the subject of a copular predicate which is complemented by a construction using the numeral \It{akta} ‘one’ and the relevant predicative adjective, while the NP of reference is in the comitative\is{case!comitative} case. An example is provided in \REF{comparingNPsEx1}. In such constructions, \It{akta} can be shortened to \It{akt}. 
...
When comparing two referents that are not considered equal, the NP of comparison is the subject of a copular predicate which is complemented by a comparative predicative adjective and the NP of reference in the elative\is{case!elative} case, as in \REF{comparingNPsEx3}. %and \REF{comparingNPsEx4}.
...
Numerals in \PS\ form a closed class and a distinct closed sub-class of adjectives. Syntactically, they are adjectives because they head an adjectival phrase\is{phrase types!adjectival phrase}; however, morphologically, they differ from other adjectives by never inflecting\is{inflection} (neither for number in predicative APs, nor for case\is{case} and number\is{number} in ellipsis constructions). 
