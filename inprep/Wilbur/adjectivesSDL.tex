...
\Bf{Adjectivals} in \PS\ are defined syntactically by their ability to be %modify the head of a nominal phrase via attribution or an entire nominal phrase via predication, as well as by being 
the head of an adjectival phrase (AP). They can be divided into four sub-categories based on both syntactic position and morphological behavior concerning inflection, as summarized in Table \vref{adjectivalsSummary}.
\begin{table}\centering
\caption{Summary of syntactic and morphological features for the three types of adjectivals}\label{adjectivalsSummary}
\resizebox{\columnwidth}{!}{
\begin{tabular}{c p{140pt} p{140pt}}%\hline
					&\MC{1}{c}{\It{syntax}}		&\MC{1}{c}{\It{morphology}}	\\\hline
\It{attributive adjectives}	&\HANG attributive position within an NP	&\HANG no inflection (except in elliptic constructions)\\\hline%, morphologically, form their own sub-group by not inflecting}
\It{predicative adjectives}	&\HANG predicative position (complement of \It{årrot} ‘be’)	&inflect for number\\\hline% (like nominals)
\It{demonstratives}		&\HANG initial attributive position within an NP	&\HANG inflect for number \& case\\\hline%, morphologically, form their own sub-group by not inflecting}
\It{numerals}			&\HANG attributive or predicative position	& never inflect\\\hline
\end{tabular}}
\end{table}

While attributive adjectives generally do not inflect, in elliptical phrases in which the head of an NP is not realized overtly, they do inflect for case and number. Predicative adjectives are marked for number, and are morphologically similar to nominals. Numerals, on the other hand, are consistently uninflected. Finally, the two types of adjectives form an open sub-class, while numerals are a closed sub-class. 

The rest of this chapter covers adjectives and numerals as follows: Section \ref{adjectivesATTR} provides a description of attributive adjectives, %(\ref{ADJinHeadlessNPs}) describes adjectives in headless NPs, 
...
Demonstratives are illustrated in Section \ref{}. 
Finally, Sections \ref{quantifiers} and \ref{numerals} cover quantifiers (a semantic sub-class of adjectives) and numerals, respectively. %, which essentially function like other adjective and predicate adjective forms. %are morphosyntactically like other adjectives and predicate adjective forms, but semantically like numerals in that they quantify the referents of the nouns they modify. 
%Finally, demonstrative adjectives, which inflect like nouns, not adjectives, are described briefly in Section \ref{demonstrativeAdjectives}.
...
\Bf{Attributive adjectives} form the head of an AP modifying the head of a nominal phrase in attributive position, and are not subject to inflectional morphology. 
...
As part of an attributive AP, an attributive adjective occurs before the head noun it modifies, but after a demonstrative, if present (cf. Section \ref{nominalPhrases} on the structure of NPs). Examples are provided in \REF{adjEx1} through \REF{adjEx3}. 
\ea\label{adjEx1}
\glll	dat lä tjähppis båtsoj ja villges åjjve dä\\
	d-a-t lä tjähppis båtsoj ja villges åjjve dä\\
	\Sc{dem}-\Sc{dist}-\Sc{nom.sg} be\BS\Sc{3sg.prs} black reindeer\BS\Sc{nom.sg} and white head\BS\Sc{nom.sg} then\\\nopagebreak
\Transl{it is a black reindeer and a white head then}{}	\Corpus{100405b}{043}
\z
\ea\label{adjEx2}
...
As the head of an AP, attributive adjectives can be modified by adverbs of grade, as illustrated by the AP \It{hoj buorak} ‘very good’ in \REF{adjEx4}.
\ea\label{adjEx4}
\glll	ja da lä årrom hoj buorak giesse\\
	ja da lä årro-m hoj buorak giesse\\
	and then be\BS\Sc{3sg.prs} be-\Sc{prf} very good summer\BS\Sc{nom.sg}\\\nopagebreak
\Transl{and it has been a very good summer}{}	\Corpus{080909}{009}
\z
...
However, as \It{njalga} ‘tasty’ in \REF{adjEx2} illustrates, not all adjectives are marked this way. Furthermore, corresponding predicate adjective forms (cf. Section \ref{predADJ} below) often also have a final \It{-s}, sometimes even to the exclusion of the attributive adjective form. Because no consistent relationship between forms with and forms without a final \It{-s} exists, it is no longer a productive way to mark or derive either attributive or predicative adjectives, and is not considered to be morphologically meaningful in the present discussion. Nonetheless, it is worth nothing that adjectival forms (both attributive and predicative) ending in \It{-s} are common. 

%One difficulty encountered in researching adjectives was that s
Note also that some attributive adjectives appear to have two possible forms. For instance, \It{guhka} and \It{guhkes} ‘long’ were both encountered in elicitation sessions with a single speaker who insisted that both forms were equally valid.\footnote{Cf. recording \hyperlink{pit080819a}{pit080819a} starting at 33m14s.} %In general there does not seem to be any reason to choose one form over another, and speakers used both forms seemingly interchangeably. 


\subsection{Attributive adjectives in elliptic constructions}\label{ADJinHeadlessNPs}
If the context of the wider discourse is sufficiently unambiguous, it is possible that the nominal head of an NP is not realized, but implied. %Because case and number marking occurs on the element of an NP which occurs at the right edge of the NP (cf. \Red{Section \ref{}}), when such elliptic constructions (sometimes called ‘headless NPs’) feature an adjective, the adjective is then the right-most element and receives case and number marking. 
...
is marked for singular number and as the object of the verb form \It{bårov} ‘I eat’ for accusative case. In \REF{adjNPheadEx2}, the subject NP also lacks an overt nominal head, but features the adjective \It{tjähppis} ‘black’ (cf. example \REF{adjEx1} above)%at the right edge
...
However, syntactically, these adjectives remain adjectives for two reasons. First, they can be modified by adverbs of grade, while nouns cannot be. Second, they generally have a referential antecedent that is the bearer of the property they denote.\footnote{The quantifier \It{aktak} ‘none, any’ can be used in an elliptic NP without a referential antecedent; cf. Section \ref{quantifiers}, specifically example \REF{noneEx3}.} 
...
\glll	lä huj tjábba, dat, dat lä jävvja\\
	...
	be\BS\Sc{3sg.prs} quite beautiful\BS\Sc{nom.sg} \Sc{dem}-\Sc{dist}-\Sc{nom.sg} \Sc{dem}-\Sc{dist}-\Sc{nom.sg} be\BS\Sc{3sg.prs} white\_reindeer\BS\Sc{nom.sg}\\\nopagebreak
\TranslLong{(it) is a quite beautiful one, it, it is a white reindeer}{(referring to ‘beautiful reindeer’)}	\Corpus{100405b}{036-037}
...
In \REF{adjPredEx1} and \REF{adjPredEx2}, for instance, the predicative adjective corresponding to the attributive adjective \It{tjähppis} ‘black’ (cf. \REF{adjEx1} above) is \It{tjáhpat}. %, a class IVa noun. %, which inflects for number. %, as  in \NOMs.\PLs\ %using the \It{-a} suffix 
%in \REF{adjPredEx2}. 
\ea\label{adjPredEx1}
...
The case could be made that they also inflect for case, although they are always in nominative case (cf. Section \ref{copulaClauses} on copula clauses). %; with this in mind, the case could be made that predicative adjectives also inflect for case (even if that case is always \NOM). 
...
\begin{tabular}{|c||c||c|c|c||l|}\hline
	...
	&\It{attributive}&\MC{2}{c|}{\It{number}}		&\It{corresp.}	&		\\%\dline
\It{no.}&\It{adjective}	&\SG	&\PL		&\It{N-class}	&\It{gloss}	\\\dline
%attr +s, pred +t
\MR{2}{*}{\It{i}}
	&tjähpis	&tjáhpat		&tjáhpada		&IVa	& ‘black’	\\\cline{2-6}%\hline
	&rusjgis	&russjgat		&russjgada	&IVa	& ‘red’	\\\dline%\cline{2-6}%\hline
%attr=pred.pl+s
\MR{4}{*}{\It{ii}}%\MR{4}{*}{\ATTRs=\PREDs.\PLs+s}
	&nievres	&nävvre		&nievre		&II	& ‘bad’	\\\cline{2-6}%\hline
	&vastes	&vasste		&vaste		&II	& ‘ugly’	\\\cline{2-6}%\hline
	&buosjes	&buossje		&buosje		&II	& ‘fearless’	\\\cline{2-6}%\hline
	&fávros	&fávvro		&fávro		&Ic	& ‘attractive’	\\\dline
%attr=pred.sg, both +s
\MR{6}{*}{\It{iii}}%\MR{6}{*}{\ATTRs=\PREDs.\SGs (-s)}
	&dájges	&dájges		&dájgesa		&IVa	& ‘cowardly’	\\\cline{2-6}%\hline
	&åvros	&åvros		&åvrosa		&IVa	& ‘nervous’	\\\cline{2-6}%\hline
	&vuoras	&vuoras		&vuorasa		&IVa	& ‘old’	\\\cline{2-6}%\hline
	
	&gujos	&gujos		&gudjosa		&IVa	& ‘frozen solid’	\\\cline{2-6}%\hline
	&luvas	&luvas		&luvvasa		&IVa	& ‘wet’	\\\cline{2-6}%\hline
	&nanos	&nanos		&nannosa		&IVa	& ‘sturdy’	\\\dline
%attr=pred.sg, pred.pl -s
\MR{1}{*}{\It{iv}}%\parbox{100pt}{\ATTRs=\PREDs.\SGs (-s); \PREDs.\PL\ w/o -s}
	&sádnes	&sádnes		&sádna		&III	& ‘true’	\\\dline%\cline{2-6}%\hline
%pred.sg+is
\MR{2}{*}{\It{v}}%\MR{2}{*}{\ATTRs=\PREDs.\SGs+s}
	&bivvalis	&bivval		&bivvala		&IVa	& ‘warm’ (weather)	\\\cline{2-6}%\hline
	&buoragis	&buorak		&buoraga		&IVa	& ‘good’	\\\dline%\cline{2-6}%\hline
%attr=pred.pl +g+s
\MR{1}{*}{\It{vi}}%
	&ånegis	&ådne		&åne			&II	& ‘short’	\\\dline%\cline{2-6}%\hline
%pred +t
\MR{4}{*}{\It{vii}}%
	&jallga	&jallgat		&jallgada		&IVa	& ‘flat’	\\\cline{2-6}%\hline
	&njuallga	&njuallgat		&njuallgada	&IVa	& ‘straight’	\\\cline{2-6}%\hline
%attr=pred.pl -s
%\MR{2}{*}{\It{viii}%
	&lägga	&lieggas		&läggasa		&IVa	& ‘warm’	\\\cline{2-6}%\hline
	&galbma	&galmas		&galbmasa	&IVa	& ‘cold’	\\\hline
\end{tabular}
\end{table}
...
Number marking like class IVa nouns.\\%\hline
\BfIt{ii}& The attributive form and the plural predicative form have the same V1 and consonant center, as opposed to the singular predicative adjective; the attributive form has a stem final \It{-s}, while the predicative forms have an open final syllable. Number marking like class I and II nouns.\\ 
\BfIt{iii}& The attributive form and the singular predicative form are syncretic and have a stem-final \It{-s}, while the plural predicative form’s stem is also identical, but marked for plural by a final \It{-a}. Number marking like class IVa nouns.\\ 
...
\BfIt{v}& The bare stem in the singular predicative form, an additional final \It{-is} for the attributive form and \It{-a} for the plural predicative form. Number marking like class IVa nouns.\\ 
...
Only one example in the corpus; number marking like a class II noun.\\ 
\BfIt{vii}& The predicative forms have a stem-final \It{-t} or \It{-s}, which is lacking in the attributive form. The plural predicative form is marked with a final \It{-a}. In the case of \It{galbma}, the attributive form and plural predicative form have the ‘strong’ stem form, as opposed to the singular predicative form, which is ‘weak’. Number marking like class IVa nouns. 
...
%\footnotetext{Cf. Section \ref{prosodicDomains} on prosodic domains.}
%%\begin{enumerate}[i]
%\begin{itemize*}
%\item{\It{i}: the adjective differs from the predicative nominal in the choice of stem allomorph concerning V1, consonant center, V2 and the final consonant.\footnote{Cf. Section \ref{} on prosodic word positions.} Only class IVa nouns.} 
%\item{\It{ii}: the adjective and the plural predicative nominal have the same V1 and consonant center, as opposed to the singular predicative nominal; the adjective has a stem final \It{-s}, while the predicative nominals have an open final syllable. Only class I and II nouns.} 
%\item{\It{iii}: the adjective and the singular predicative nominal are syncretic and have a stem-final \It{-s}, while the plural predicative nominal stem is also identical, but marked for plural by a final \It{-a}. Only class IVa nouns.} 
%\item{\It{iv}: the adjective and the singular predicative nominal are syncretic and have a stem-final \It{-s}, while the plural predicative nominal is marked by a final \It{-a} instead of the final \It{es} in the other forms. Only one example in the corpus, and a class II noun.} 
%\item{\It{v}: the bare stem in the singular predicative nominal, an additional final \It{-is} for the adjective and \It{-a} for the plural predicative form. Only class IVa nouns.} 
%\item{\It{vi}: the singular and plural predicative nominals differ only in the choice of stem allomorph (in the consonant center), while the adjective is in the ‘weak’ grade (like predicative plural) but with a stem-final \It{gis}.\footnote{Historically, the \It{-g-} could have been a nominalizer, which was then further derived to the adjective form by the (historical) \It{-s} adjectivizer.} Only one example in the corpus, and a class II noun.} 
%\item{\It{vii}: The predicative nominals have a stem-final \It{-t} or \It{-s}, which is lacking in the adjective. The plural predicative nominal is marked with a final \It{-a}. In the case of \It{galbma}, the adjective and plural predicative nominal have the ‘strong’ stem form, as opposed to the singular predicative nominal. Only class IVa nouns.} 
%%\item{\It{viii}: The predicative nominals have a stem final \It{-s}, which is lacking in the adjective. The plural predicative nominal is marked with a final \It{-a}. Only class IVa nouns.} 
%%\end{enumerate}
%\end{itemize*}

Despite the similarities with nouns described above, it is important to point out that there are a number of predicative adjectives which do \It{not} inflect for number. Moreover, this lack of number marking cannot be assigned to any specific noun class, % described in Section \ref{nounClasses} above because the predicative forms are not marked for number, 
particularly since noun classes with similar phonological forms exhibit clear number marking strategies. Examples of such predicative adjectives are presented in Table \vref{noPredAdjTable}. 
...
\begin{tabular}{|c|c||c|c||l|}\hline
	&\It{attributive}	&\MC{2}{c||}{\It{predicative adjective}}			&\\
\It{no.}&\It{adjective}	&\SG	&\PL				&\It{gloss}	\\\dline
\MR{4}{*}{\It{viii}}
	&tjábba	&tjábbe		&tjábbe		& ‘beautiful’	\\\cline{2-5}%\hline
	&guhka	&guhke		&guhke		& ‘long’		\\\cline{2-5}%\hline
	&unna	&unne		&(unne)		& ‘small’		\\\cline{2-5}%\hline
	&smáva	&-			&smáve		& ‘small’		\\\dline
\MR{5}{*}{\It{ix}}
	&tjåskes	&tjåskes		&tjåskes		& ‘cold’ (weather)	\\\cline{2-5}%\hline
	&låjes	&låjes		&låjes		& ‘tame’	\\\cline{2-5}%\hline
%\MR{3}{*}{\It{x}
	&gårå	&gårå		&gårå		& ‘bad’	\\\cline{2-5}%\hline
	&räkta	&räkta		&räkta		& ‘correct’	\\\cline{2-5}%\hline
	&buorre	&buorre		&buorre		& ‘good’	\\\hline
\end{tabular}
\end{table}
%\FloatBarrier
...
\begin{longtable}{c p{325pt}}
%\BfIt{subgroup}	& description\\
\BfIt{viii}& The attributive form ends in \It{-a}, while the predicative form ends in \It{-e}.\footnote{Cf. Section \ref{smallADJs} for more details on \It{unna}/\It{unna} and \It{smáve}/\It{smáve}, the words for ‘small’.} \\
...
\marginpar{what is wrong with this section? where is the pagebreak?!?}
...
Furthermore, the paradigms are divided into subgroupings (each marked with a Roman numeral) based on suffix allomorph patterns. The third and fifth columns in Table \ref{compSuperlADJsTable} provide the singular predicative comparative and superlative adjectives, respectively, while the fourth and sixth columns only indicate the suffix used to mark the plural predicative comparative and superlative adjectives, respectively. The allomorphic alternations in the superlative suffix for subgroupings \It{iii} and \It{iv} are indicated by italics. 
...
\resizebox{\columnwidth}{!}{
\begin{tabular}{|c|c|cc|cc|l|}\hline
...
\MR{3}{*}{\It{i}}
	&nävvre		&nievre-p		&-a			&nievre-mus		&-a	& ‘bad’	\\\cline{2-7}%\hline
	&ådne		&åne-p		&-a			&åne-mus			&-a	& ‘short’	\\\cline{2-7}%\hline
	&guhke		&guhke-p		&-a			&guhke-mus		&-a	& ‘long’	\\\dline%\cline{2-7}%\hline
\MR{4}{*}{\It{ii}}
	&tjábbe		&tjábba-p		&-a			&tjábba-jmus		&-a	& ‘beautiful’	\\\cline{2-7}%\hline
	&vasste		&vasste-p		&-a			&vasste-jmus		&-a	& ‘ugly’	\\\cline{2-7}%\hline
	&gårå		&gårå-p		&-a			&gårå-jmus		&-a	& ‘bad’	\\\cline{2-7}%\hline
	&fávvro		&fávro-p		&-a			&fávro-jmus		&-a	& ‘attractive’	\\\dline%\cline{2-7}%\hline
\MR{2}{*}{\It{iii}}
	&luvas		&luvasu-p		&-o			&luvasu-mos	&-bmus-a	& ‘wet’	\\\cline{2-7}%\hline
	&garras		&garrasu-p	&-o			&garrasu-mos	&-bmus-a	& ‘hard’	\\\dline%\cline{2-7}%\hline
%låjes			&låjes-u-p		&-o			&låjes-u-mos		&-musa	& ‘tame’	\\\cline{2-7}%\hline%JW: DS provides conflicting data!
\MR{6}{*}{\It{iv}}
	&nanos		&nanosu-p	&-a			&nanosu-mos	&-bmus-a	& ‘strong’	\\\cline{2-7}%\hline
	&bivval		&bivvalu-p		&-a			&bivvalu-mos	&-bmus-a	& ‘warm’	\\\cline{2-7}%\hline
	&tjáhpat		&tjáhpadu-p	&-a			&tjáhpadu-mus	&-bmus-a	& ‘black’	\\\cline{2-7}%\hline
	&galmas		&galbmasu-p	&-a			&galbmasu-mos	&-bmus-a	& ‘cold’	\\\cline{2-7}%\hline
	&vuoras		&vuorasu-p	&-a			&vuorasu-mos	&-bmus-a	& ‘old’	\\\cline{2-7}%\hline
	&njuallgat		&njuallgadu-p	&-a			&njuallgadu-mos	&-bmus-a	& ‘correct’	\\\hline
...
Essentially, the superlative suffix always forms the final foot of a word, and thus is the location for consonant gradation alternations. If a final, odd syllable is present (e.g., for the plural predicative form), then the \mbox{\It{-bmus-}} allomorph is chosen. Cf. Section \ref{prosodicDomains} on prosodic domains and Section \ref{Cgrad} on consonant gradation. 

Examples for C/S adjectives in attributive position can be found in \REF{compATTRADJex1} and \REF{superlATTRADJex1}, respectively. Instances for predicative usage can be found in \REF{comparingNPsEx3} and \REF{comparingNPsEx5} in the Section \ref{comparingNPs}.
...
%Note that demonstrative pronouns are identical to demonstrative adjectives modifying a noun; cf. Section \ref{demonstrativeAdjectives}.
%\marginpar{should this perhaps be part of section on demonstrative pronouns?}
\korr{009}Demonstratives %described in Section \ref{demonstrativePronouns} and inventoried in Table \vref{DemPronTable} on page \pageref{DemPronTable} 
modify a noun phrase by further specifying the head noun by indicating the distance of the referent relative to the speaker. Just as with demonstrative pronouns, the corpus data indicates that there is a three-way distinction between referents close to the speaker (proximal), those away from the speaker (distal), and those particularly far away (remote). 
...
Unlike adjectives, demonstratives always \korr{024}agree with the noun they modify in number (singular and plural, but not dual) and in case. %, and indicate distance of the referent relative to the speaker (just as when they are demonstrative pronouns). %The demonstrative adjectives are identical to the demonstrative pronouns described in Section \ref{demonstrativePronouns} and inventoried in Table \vref{DemPronTable} on page \pageref{DemPronTable}. 
...
\glll	dajd gulijd giesijmä tjielkajn dik\\
	...
	\Sc{dem}-\Sc{dist}-\Sc{acc.pl} fish-\Sc{acc.pl} pull-\Sc{1pl.pst} sled-\Sc{com.sg} to\_here\\\nopagebreak
...
\glll	muvne lä binna vuopta\\
	...
	\Sc{1sg.iness} be\BS\Sc{3sg.prs} little\_bit hair\BS\Sc{nom.pl}\\\nopagebreak
...
Note, however, that \It{gajk} ‘all’ can optionally be marked for \PL\ in attributive position by adding the suffix \It{-a}, as shown in \REF{quantifierEx3}.
...
it inflects for case and number (as with other attributive adjectives). This is illustrated by \It{enabu} ‘more’ in \REF{quantifierEx6}, %which is inflected for \ACCs.\SGs\ case, and 
...
\glll	mana tjasskit dajt åjvijd ja gajkajd duhku\\
	mana tjasski-t d-a-jt åjvi-jd ja gajk-ajd duhku\\
	go\BS\Sc{2sg.imp} throw-\Sc{inf} \Sc{dem}-\Sc{dist}-\Sc{acc.pl} head-\Sc{acc.pl} and all-\Sc{acc.pl} over\_there\\\nopagebreak
...
%UM: 
%It is likely based on a derived noun using the numeral \It{akta} ‘one’ and the nominalizing suffix \It{-k}; however, as illustrated by the examples in \REF{noneEx1} and \REF{noneEx2}, it heads an attributive AP and does not inflect for case and number, unless it is in an elliptic NP, %the head of an NP, 
...
\glll	itjij almatj åbbå hålå aktagav\\
	...
	\Sc{neg}-\Sc{3sg.pst} person\BS\Sc{nom.sg} at\_all say\BS\Sc{conneg} none-\Sc{acc.sg}\\\nopagebreak
...
%is nominalized 
%Quantifiers have corresponding predicate nominals forms, as well (like other adjectives). The predicative adjective forms can be a nominalized form of the quantifier,
%using the derivational suffix \It{-k} in predicative position; 
...
%
%It is not clear at this point what determines whether a quantifier as the head of an NP is nominalized, and this question must be left to future research.



\section{Numerals}\label{numerals}\index{numerals}\index{parts of speech!numerals}
\Bf{Numerals} %and non-numeral quantifiers 
in \PS\ form a closed class and a distinct sub-category of adjectives. Syntactically, they are adjectives because they head an adjectival phrase; however, morphologically, they differ from other adjectives by never inflecting (neither for number in predicative APs, nor for case and number in ellipsis constructions). 
...
Instead, numerals are consistent in form, regardless of being in attributive or predicative position. %\footnote{Quantifiers are similar to numerals semantically in that they also quantify the referent of the noun they modify; however, quantifiers behave morphosyntactically like adjectives, and are thus discussed in Section \ref{quantifiers} in the chapter on adjectives.} 
...
\MC{1}{c|}{}&\It{cardinal}&&\It{ordinal}	&\MC{1}{c}{} \\\dline%\cline{1-2}\cline{4-5}\cline{1-2}\cline{4-5}%\dline
...
100	&tjuohte		&&?			&100\superS{th}	\\\cline{1-2}\cline{4-5}%\hline
1000	&tuvsan		&&?			&1000\superS{th}	\\\hline%\cline{1-2}\cline{4-5}%\hline
\end{tabular}
\end{table}

\subsubsection{Derivation and suppletion in ordinal numerals}\label{ordinalNums}
In general, ordinal numerals, which are listed on the right side of Table \vref{basicNumsTable}, can be derived from the corresponding cardinal numeral by replacing the second vowel and any final consonant with the suffix \It{-át} \korr{076}(and its allomorph \It{-et} in \It{giehtjet} ‘seventh’).  In addition, the weak stem allomorph is selected and umlaut of V1 occurs, if applicable.\footnote{Cf. Section \ref{morphophonology} on stem allomorphy.} %and subjecting the stem to the consonant gradation and/or umlaut, when relevant. 
...
\glll	… gokt lij dánne giehtjavlåg jage maŋŋus\\
	...
	{} how be-\Sc{3sg.pst} here seven-ten year-\Sc{nom.pl} ago\\\nopagebreak
...
One method appends the relevant numeral representing the ‘ones-digit’ to the multiple of ten, while \It{lågev} ‘ten’ is shortened to \It{låg}; this is illustrated in Figure \vref{twoDigitNumsA}. %\It{aktalågakta} ‘eleven’ (literally ‘one-ten-one’), \It{gålbmålågguhta} ‘thirtysix’ (literally ‘three-ten-six’), \It{åktselåkgakktse} ‘ninetyeight’. 
\begin{figure}\centering
\begin{tabular}{c| c| c c}
...
Examples of two-digit numerals from the corpus are found in \REF{complexNumEx2} and \REF{complexNumEx3}.
...
\glll	dä lij del tjuojgadam ja gåddam nälljalåkgakktse stalpe sájtejna\\
	...
	then be-\Sc{3sg.pst} then ski-\Sc{prf} and slay-\Sc{prf} four-ten-eight wolf-\Sc{gen.pl?} spear-\Sc{com.sg} \\\nopagebreak
...
%\subsection{Case and number marking on numerals}\label{caseNumberOnNum}
\subsection{Numerals and morphosyntax}\label{caseNumberOnNum}
Numerals are generally not subject to inflectional morphology, even in elliptic constructions and in predicative position. %, i.e., unlike nouns, they do no inflect for case or number, and unlike adjectives, they do not inflect for \marginpar{update this comparison to ADJ if necessary!}attribution or predication. 
This is illustrated by the examples \REF{complexNumEx1} through \REF{complexNumEx3} above as well as in \REF{numeralEx2} through \REF{numeralEx4} below. 
%UM notes: \REF{numeralEx2} and \REF{numeralEx4} do not illustrate missing inflection on the ordinal numeral, because an adjective would not have inflection here either.
%JW: ?? I don’t understand - there is no inflectional morphology present, thus i
...
\Transl{and the third (net) then goes to Stutja}{}	\Corpus{090702}{026-027}
\z

However, there are at least two exceptions. First, the numeral \It{akta} ‘one’, which inflects for \ACCs.\SGs\ case when modifying a noun, as illustrated by \REF{numOneEx1}, as well as when it is in a headless elliptical construction, as in \REF{numOneEx2}.%\marginpar{footnote motivated by UM’s comment, not sure though if it’s a good idea}
