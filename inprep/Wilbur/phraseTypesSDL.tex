...
\begin{tabular}{|c|c|c|c|c|c|}\hline
	&			&\It{argument/adjunct/}	&			&			&\\%\dline
	&\It{predicate}	&\It{complement}	&\It{modifier in NP}&\It{modifier in AP}&\It{section}\\\dline
VC	&\CH		&				&			&			&\ref{verbComplex}	\\\hline%predicate
NP	&\CH		&\CH			&\CH		&			&\ref{nominalPhrases}	\\\hline%argument, adjunct, modify NP (gen)
AP	&			&\CH			&\CH		&			&\ref{adjectivalPhrases}	\\\hline%modify NP-head, modify NP
AdvP&			&\CH			&			&\CH		&\ref{adverbialPhrases}	\\\hline%adjunct, modify AP-head
PP	&			&\CH			&			&			&\ref{postpositionalPhrases}	\\\hline%adjunct
\end{tabular}}
\end{table}
...
The \PS\ \Bf{verb complex} (abbreviated ‘VC’) consists minimally of a finite verb, and maximally of a finite verb and one or two non-finite verb forms. 
...
The imperative only inflects for number.\korr{029} %, which cannot be regarded as an agreement suffix in this case because imperatives do not have subject arguments. %NOT RELEVANT
%
...
\item{grammatical verbs (the negation verb, the aspectual auxiliary verb \It{årrot} ‘be’, and the modal verbs\footnote{Cf. Section \ref{multiVdeclarativeClauses} for more on these grammatical verbs.})}
\end{itemize} 
...
\MR{2}{*}{1}&\It{finite verb}		& & \\%\hline
&\It{(lexical/copula)}	& & 	\\\dline%\cline{2-4}
%&copula verb	& & 	\\\dline
%&&&\\\hline
\MR{2}{*}{2}&\It{finite verb}		&\MR{2}{*}{\PLUS} &\It{non-finite form} \\%\hline
&\It{(grammatical)}		& &\It{(lexical/copula)} \\\hline
&negation verb	&\PLUS & \CONNEGs	\\\cline{2-4}%\hline
&aspectual auxiliary&\PLUS & \PRFs{\tiny\ or }\PROGs	\\\cline{2-4}%\hline
&modal verb	&\PLUS & \INFs	\\\hline
\end{tabular}}
\resizebox{\textwidth}{!}{
...
\MR{2}{*}{3}&\It{finite verb}		&\MR{2}{*}{\PLUS} &\It{non-finite form}	&\MR{2}{*}{\PLUS} &\It{non-finite form} \\%\hline
&\It{(grammatical)}		& &\It{(grammatical)}		& &\It{(lexical/copula)} \\\hline
&negation verb			&\PLUS & asp. auxiliary/\CONNEGs		&\PLUS & \PRFs{\tiny\ or }\PROGs	\\%\hline
&					&\PLUS & modal/\CONNEGs		&\PLUS & \INFs	\\\cline{2-6}
&aspectual auxiliary		&\PLUS & modal/\PRFs{\tiny\ or }\PROGs			&\PLUS & \INFs	\\\hline%\cline{2-6}%\hline
...
and consists of the finite negation verb and the lexical verb \It{dáhkat} ‘make, do’ in its connegative form, but is split by the particle \It{dä} and the NP argument \It{aktagav}. The second VC is the verb \It{vähtjat} ‘fetch’, now in its \Sc{1sg.prs} finite form. 
\ea\label{VCex3}
...
\glll	jus stalpe lin båhtam elo sissa, dä vuolgin ja vitjin davva\\
	...
	if wolf\BS\Sc{nom.pl} be-\Sc{3pl.pst} come-\Sc{prf} reindeer\_herd\BS\Sc{gen.sg} into then drive-\Sc{3pl.pst} and fetch-\Sc{3pl.pst} \Sc{dem}-\Sc{dist}-\Sc{acc.sg}\\\nopagebreak
...
Finally, the clauses in \REF{VCex6} through \REF{VCex8} provide examples of VCs with three verb forms. In \REF{VCex6}, the VC consists of the finite negation verb in \Sc{1sg.prs} (\It{iv}), the aspectual auxiliary \It{årrot} in connegative form (\It{lä}), and the lexical verb \It{gullat} ‘hear’ in its perfect form (\It{gullam}). The VC in example \REF{VCex7} contains the finite negation verb \Sc{3sg.prs} (\It{ij}), the modal verb \It{máhttet} in connegative form (\It{máhte}), and the lexical verb \It{adnet} ‘have’, its infinitive form. Finally, in \REF{VCex8}, the finite aspectual auxiliary \It{lev} combined with the perfect form of the modal verb \It{máhttet} ‘can’ and the infinite complement \It{ságastit} ‘speak’ constitute the VC.
\ea\label{VCex6}
...
\Transl{… since I haven’t heard that at all}{}	\Corpus{090702}{203}
...
\Bf{Nominal phrases} (abbreviated ‘NP’) in \PS\ are divided into two groups:
\begin{itemize}
\item{full NPs}
\item{pronouns}
%\item{predicate APs}
...
\end{figure}
Either a noun or a pronoun forms the head of an NP. The demonstrative, the nominal and any attributive reflexive pronoun always inflect for case and number, while generally the other modifiers do not. Other modifiers may be an adjectival phrase, a numeral or an NP in genitive case. \korr{049}An intensifier in the form of a reflexive pronoun\footnote{Cf. Section \ref{reflexivePronouns} on reflexive pronouns.} 
...
Some examples for possible NP structures are found below.%, and in Section \ref{} for relative clauses. 
...
\glll	ja dä vuojnav Tjeggelvasav\\
	...
	and then see-\Sc{1sg.prs} Lake\_Tjeggelvas-\Sc{acc.sg}\\\nopagebreak
...
\glll	ja dä såkoj dat ello\\
	...
	and then drown-\Sc{3sg.pst} \Sc{dem}-\Sc{dist}-\Sc{nom.sg} reindeer\_herd\BS\Sc{nom.sg}\\\nopagebreak
...
\glll	dáj Skailej ello såkoj\\
	...
	\Sc{dem}-\Sc{prox}-\Sc{gen.pl} Skaile-\Sc{gen.pl} reindeer\_herd\BS\Sc{nom.sg} drown-\Sc{3sg.pst}\\\nopagebreak
...
as in \REF{NPstructureEx6}, in which the noun referring to nets is not realized, so \It{gålbmad} ‘third’ is the NP. %In \REF{NPstructureEx7}, . 
\ea\label{NPstructureEx6}
\glll	ja gålmád sjadda dä Stutjaj\\
	ja {[gålmád]\subNP\sub{\It{1}}} sjadda dä {[Stutja-j]\subNP\sub{\It{2}}}\\
	and third become\BS\Sc{3sg.prs} then Stutja-\Sc{ill.sg}\\\nopagebreak
\Transl{and the third one is then placed at Stutja}{(referring to ‘fishing net’)}	\Corpus{090702}{026-027}
\z
With the exception of \It{akta} ‘one’, numerals do not inflect for case and number.\footnote{Cf. Section \ref{numerals} for more on numerals.}
...
There are many examples in the corpus of NPs consisting only of a demonstrative, as in \It{dajd} in \REF{NPstructureProEx1}, or other pronoun, as in \It{mån} in \REF{NPstructureProEx2}. %Here, the first NP \It{dajd} ‘those’ consists only of the determiner. %, while the head noun referring to fish is not realized. 
...
\glll	mån mähtiv ráhpat uksav\\
	{[mån]\subNP\sub{\It{1}}} mähti-v ráhpa-t {[uksa-v]\subNP\sub{\It{2}}}\\
...
Nominal phrases, %\footnote{Cf. Section \ref{nominalPhrases} for more details about nominal phrases.} 
particularly referring to time or place, and thus inflected for one of the locative cases (illative, inessive or elative), are often used as temporal or locational adverbials, as in \It{giesen} ‘in summer’ in \REF{ADVnounsEx1}, and in \It{hieman} ‘at home’ in \REF{ADVnounsEx2}, respectively. Furthermore, the word \It{vahkov} ‘week’ in \REF{ADVnounsEx2} shows that an accusative NP can function as a temporal adverbial indicating a period of time. 
...
\glll	ja Henning lij jou hieman urrum vahkov\\
	ja {[Henning]\subNP\sub{\It{1}}} li-j jou {[hiema-n]\subNP\sub{\It{2}}} urru-m {[vahko-v]\subNP\sub{\It{3}}}\\
...
\Bf{Adjectival phrases} (abbreviated ‘AP’) in \PS\ are divided into three groups based on the type of adjective required as head: 
\begin{itemize}
...
\begin{figure}\centering
\begin{tabular}{l l}
attributive AP:	&\verb|[[|(AdvP) \PLUS\ attributive adjective]\subAP \\%\[ added so that the second [ is typeset at beginning of line
predicative AP:	&\verb|[[|(AdvP) \PLUS\ predicative adjective]\subAP \\%\[ added so that the second [ is typeset at beginning of line
numeral AP:	&\verb|[[|(AdvP) \PLUS\ numeral]\subAP \\%\[ added so that the second [ is typeset at beginning of line
\end{tabular}
...
The data is quite limited concerning which type of AdvPs are acceptable, and future research is needed in this respect. Examples from the corpus include %\marginpar{more AdvP in APs??} 
...
\glll	da lä urrum huj buorak giesse\\
	...
	then be\BS\Sc{3sg.prs} be-\Sc{prf} really good summer\BS\Sc{nom.sg}\\\nopagebreak
\Transl{it has been a really good summer}{}	\Corpus{080909}{009}
\z
...
Finally, examples of the numeral APs are provided in \REF{APstructureEx5} through \REF{APstructureEx7}. In the first example, \It{guäkte} ‘two’ modifying the NP \It{dåpe} ‘houses’ is provided in \REF{APstructureEx5}. 
...
\z
The example in \REF{APstructureEx6}\footnote{This example is also provided in example \REF{veryFirstEx} in Section \ref{caseNumberOnNum}, but is repeated here for convenience.} 
is noteworthy because it shows how a numeral can be inflected as a superlative; here the ordinal numeral \It{vuostas} is in the superlative form \It{vuostamos} meaning ‘very first’, and modifies the NP \It{guhkajuolgak} ‘moose’. 
...
\glll	jus galga njuallga dajd njuovvat dä galga dajd valldet ulgus åvdål gádtsastij\\
	...
	if will\BS\Sc{3sg.prs} correct \Sc{dem}-\Sc{dist}-\Sc{acc.pl} slaughter-\Sc{inf} then will\BS\Sc{3sg.prs} \Sc{dem}-\Sc{dist}-\Sc{acc.pl} take-\Sc{inf} out before hang\_up-\Sc{3sg.pst}\\
...
An \Bf{adverbial phrase} (abbreviated ‘AdvP’) has an adverb as its head, and can potentially be modified by a further AdvP, as illustrated in Figure \vref{AdvPstructures}
\begin{figure}\centering
\begin{tabular}{l }
...
\glll	dalloj dä lij, manaj buoragit\\
	...
	at\_that\_time then be-\Sc{3sg.pst} go-\Sc{3sg.pst} good-\Sc{advz}\\\nopagebreak
...
A \Bf{postpositional phrase} (abbreviated ‘PP’) is headed by a postposition, which is always preceded by an NP complement. Any components in this complementing NP which are subject to case inflection inflect for genitive (as well as number). This structure is illustrated in Figure \vref{PPstructure}. 
...
a noun with a demonstrative in \REF{PPex1}, a single noun in \REF{PPex2}, a demonstrative in \REF{PPex3}, a personal pronoun in \REF{PPex4}, an interrogative pronoun in \REF{PPex5}, and an interrogative NP \REF{PPex6}.
...
Postpositional phrases %\footnote{Cf. Section \ref{postpositionalPhrases} for more details about postpositional phrases.} 
...
\glll	dä Kataridna ja månnå dan giedge nanne pruvkojin ståhkåt\\
	...
	then Katarina\BS\Sc{nom.sg} and \Sc{1sg.nom} \Sc{dem}-\Sc{dist}-\Sc{gen.sg} rock\BS\Sc{gen.sg} on used\_to-\Sc{3pl.pst} play-\Sc{inf}\\\nopagebreak
