...
\section{Verb complex}\label{verbComplex}\is{verb complex}
...
\begin{itemize}
\item{lexical verbs and the copula verb\is{copular verb} \It{årrot} ‘be’}
...
the selection of each non-finite form is determined by the type of verb governing it: the verb of negation triggers the connegative\is{connegative} form, the aspectual auxiliary verb triggers either the perfect or the progressive form, and the modal verbs trigger the infinitive form.
...
\section{Nominal phrases}\label{nominalPhrases}\is{nominal phrase}
...
\section{Adjectival phrases}\label{adjectivalPhrases}\is{adjectival phrase}
...
\item{predicate APs (headed by predicative adjectives\is{adjective!predicative})}% as the complement of the copula verb \It{årrot} ‘be’}
\item{numeral APs (headed by numerals\is{numeral})}%
\end{itemize}
%In all cases, APs are used as modifiers. %the head noun of an NP. 
In the first case, the attributive AP is syntactically embedded in an NP\is{nominal phrase} whose head noun it modifies. In the second case, the predicative AP ascribes the property it concerns to the entire NP which is the subject of the copula clause which the AP is embedded in. Numeral APs can occur both attributively and predicatively. 
...
Adverbial phrases\is{adverbial phrase} are not common in APs. %, and only certain adverbial phrases can be a constituent in an AP. 
...
\section{Adverbial phrases}\label{adverbialPhrases}\is{adverbial phrase}
...
\section{Postpositional phrases}\label{postpositionalPhrases}\is{postpositional phrase}\is{adpositional phrase}
...
The complement in a PP can be any valid nominal phrase. A number of examples for various NPs\is{nominal phrase} complementing the head of a PP are provided below: %and marked in \Bf{bold} face: 
