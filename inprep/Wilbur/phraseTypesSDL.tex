...
\section{Verb complex}\label{verbComplex}\is{phrase types!verb complex}
%\Blue{Here comes the verb complex!!!!!!!!!!!!!!!} oder auch nicht.
The \PS\ verb complex (abbreviated ‘VC’) consists minimally of a finite verb, and maximally of a finite\is{finite verb} verb and one or two non-finite verb forms\is{non-finite verb form}. 
With the exception of the imperative, the finite verb inflects for tense\is{tense} or mood\is{mood}, number\is{number} and person\is{person}, and agrees\is{agreement} with the subject. 
...
\begin{itemize}
\item{lexical verbs and the copular verb\is{copular verb} \It{årrot} ‘be’}
\item{grammatical verbs (the negation verb, the aspectual auxiliary verb \It{årrot} ‘be’, and the modal verbs; cf. \SEC\ref{multiVdeclarativeClauses}).}
\end{itemize} 
...
the selection of each non-finite form is determined by the type of verb governing it: the verb of negation triggers the connegative\is{non-finite verb form!connegative} form, the aspectual auxiliary verb triggers either the perfect or the progressive form, and the modal verbs trigger the infinitive form.
...
For instance, the examples in \REF{VCex1} and \REF{VCex2} each feature a VC consisting solely of a finite verb. In \REF{VCex1} it is the singular imperative form of the lexical verb \It{vädtjat} ‘fetch’, while in \REF{VCex2} it is the encliticized \Sc{3sg.prs} form of the copular verb. %, which consists exclusively of the \Sc{3sg.prs} encliticized form of the verb \It{årrot} ‘be’. 
\ea\label{VCex1}
...
\section{Nominal phrases}\label{nominalPhrases}\is{phrase types!nominal phrase}
...
Less commonly, an adjectival phrase\is{phrase types!adjectival phrase} can be in an elliptical construction, either with or without a demonstrative (cf. \SEC\ref{ADJinHeadlessNPs}). In the absence of a head noun, the adjective in the AP is the host for case\is{case} and number\is{number}, and is morphologically a nominal. In the example in \REF{NPstructureEx8}, the head noun referring to a piece of candy is not realized, and the NP consists only of the adjective \It{rupsisav} ‘red’, which inflects for case and number.\footnote{Cf. \SEC\ref{ADJinHeadlessNPs} for more details about adjectives in elliptical structures.} 
...
Finally, a relative clause\is{relative clause} can also occur after the head as part of the NP. An example is provided in \REF{NPstructureRelClEx1}; see \SEC\ref{relativeClauses} for more on relative clauses. 
...
\subsection{NPs in adverbial function}\label{ADVnouns}\is{adverbial}
...
\section{Adjectival phrases}\label{adjectivalPhrases}\is{phrase types!adjectival phrase}
...
Adverbial phrases\is{phrase types!adverbial phrase} are not common in APs. %, and only certain adverbial phrases can be a constituent in an AP. 
...
While the data in the corpus are quite limited, it appears that APs headed by an attributive adjective can be used adverbially.\is{adverbial} This is illustrated by \It{njuallga} ‘correct’ in \REF{ADVadjectivesEx1}. %and \It{spájta} ‘quick’ in \REF{ADVadjectivesEx2}.%quick-ATTR = spájtas, quick-ADV = spájta
...
\section{Adverbial phrases}\label{adverbialPhrases}\is{phrase types!adverbial phrase}
...
\section{Postpositional phrases}\label{postpositionalPhrases}\is{phrase types!postpositional phrase}\is{phrase types!adpositional phrase}
...
The complement in a PP can be any valid nominal phrase. A number of examples for various NPs\is{phrase types!nominal phrase} complementing the head of a PP are provided below: %and marked in \Bf{bold} face: 
