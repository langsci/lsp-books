...
In the following, derivational meanings are assigned to suffixes for simplicity in classification; however, as with inflectional\is{inflection} suffixes, derivational suffixes coincide with non-linear morphology\is{non-linear morphology} when the derivational base is subject to non-linear morphological alternations. %As can be expected cross-linguistically, derivational suffixes occur before inflectional
...
\subsection{The diminutive suffix \It{-tj}}\label{dim}\is{diminutive}
...
\subsection{The general nominalizer suffix \It{-k}}\label{nmlz2}\is{nominalizer}
...
\subsection{The action nominalizer suffix \It{-o}}\label{nmlz1}\is{nominalizer}
...
\subsection{The agent nominalizer suffix \It{-däddje}}\label{agentNMLZ}\is{nominalizer}
...
The base is typically a verb\is{verb}, but can be a noun, as in \REF{agentNMLZex4}. The stem of the derived agent noun is in the weak grade\is{weak grade}. 
...
\subsection{The state nominalizer suffix \It{-vuohta}}\label{vuohta}\is{nominalizer}
...
\subsection[The diminutive verbalizer suffix \It{-tj}]{The diminutive verbalizer suffix \It{-tj} and the complexities of \PS\ derivational verb morphology}\label{verbDIM}\is{verbalizer}\is{diminutive}
...
\subsection{The verbal derivational suffix \It{-st}}\label{vblzST}\is{verbalizer}
...
\subsection{The verbal derivational suffix \It{-d}}\label{vblzD}\is{verbalizer}
...
\subsection{The verbal derivational suffix \It{-dall}}\label{vblzDALL}\is{verbalizer}
...
\subsection{Passivization with the derivational suffix \It{-duvv}}\label{VdervPassives}\is{passive}
Transitive\is{transitive} verbs can be passivized using the suffix\is{suffix!derivational} \It{-duvv} (glossed as \Sc{pass}). %\marginpar{or only \Bf{-uvvu}, with \Bf{-t-} as \Sc{inf}?}, %JW: decided to ignore this possibility
...
\section{Adjectival derivation}\label{adjDerivation}\is{adjectivalizer}
...
The basic ordinal\is{ordinal} numerals can be derived by applying the derivational suffix \It{-át} (or its allomorph \It{-et}) %\TILDE\It{-et} %just gonna consider -et an allomorph
...
\section{Adverbial derivation}\label{advDerivation}\is{adverbializer}
