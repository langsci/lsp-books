...
\fbox{\includegraphics[width=1\textwidth]{images/motormännens20121115.jpg}}}
...
One of the outcomes of this \WLP\ was a working orthography for \PS. 
...
I have attempted to adopt this working orthographic standard in writing \PS\ data in this description and the transcriptions provided in the accompanying documentation. This orthography uses the Swedish alphabet and many Swedish sound-to-grapheme correspondences, but also resembles to some extent Lule Saami orthographic systems, particularly the most recent one as found in \citet{KorhonenO2005}. To help understand the orthography, the correlations between sounds and graphemes are discussed here and listed in Table \vref{orthTableV} for vowels and Table \ref{orthTableC} on pages \pageref{orthTableCbegin} and \pageref{orthTableCend} for consonants. %\marginpar{this table was just copied in - it needs to be re-worked, checked!!!}
\korr{057}Chapter \ref{csANDvs} describes the segmental phonology represented by these graphemes in detail. 

It is important to note that this is a \It{working} orthography, i.e., it is still subject to inconsistencies and potentially to further refinement. Moreover, the following description and the implementation of the orthography is based on my interpretation of the recurring patterns used by the \WLP. While I generally use the spellings found in the \WLP’s wordlist, some deviations may be found in this description and the accompanying transcriptions; these can be due to a variety of factors such as changes to the word list as it was developed, deviating analyses on my own behalf, or simple inconsistencies in spelling (a natural occurrence in the development of an orthography). However, I alone am ultimately responsible for the orthographic choices in the present work. 

%%new command for this section on orthographic examples
...
\caption{Vowel phonemes and their corresponding graphemes in the working-version of the \PS\ orthography}\label{orthTableV}
\begin{tabular}[t]{| c  c | l |}\hline
%\MC{3}{|c|}{\BfIt{vowels}}\\\hline
\It{phoneme}	&\It{letter or}	&		\\
\It{(IPA)}		&\It{letters}	&\It{comments/context}	\\\hline%& example \\\hline
...
\IPA{u͡a}	&\Grapheme{ua}	& default grapheme	\\*
...
\caption{Consonant phonemes and their corresponding graphemes in the working-version of the \PS\ orthography\label{orthTableC}}\\%longtable-caption a bit special: needs \label{} inside caption, needs \\ at end of line, comes after \begin{longtable}{cclr}...
\hline
%\MC{3}{|c|}{\BfIt{consonants}}\\\hline
\It{phoneme}	&\It{letter or}	&		\\
\It{(IPA)}		&\It{letters}	&\It{comments/context}	\\\hline%& example \\\hline
\endhead
\IPA{p}	&\Grapheme{b}		& default grapheme \\*
		&\Grapheme{p}		& word-finally; in C-cluster except after sonorant C\\\hline
\IPA{ʰp}	&\Grapheme{hp}	& default grapheme	\\*
		&\Grapheme{p}		& after sonorant C in C-center	\\\hline%& \It{gummpe} ‘wolf’ \\\hline
\IPA{pː}	&\Grapheme{bb}	& default grapheme \\*%\hline
		&\Grapheme{pp}	& before unvoiced or preaspirated C \\\hline %; (inconsistently intervocalically)	\\\hline
\IPA{ʰpː}	&\Grapheme{hpp}	& default grapheme	\\\hline
\IPA{t}	&\Grapheme{d}		& default grapheme \\*
		&\Grapheme{t}		& word-finally; in C-cluster except after sonorant C\\\hline
\IPA{ʰt}	&\Grapheme{ht}	& default grapheme	\\*
		&\Grapheme{t}		& after sonorant C in C-center \\\hline%& \It{virrtit} ‘must’ \\\hline
\IPA{tː}	&\Grapheme{dd}	& default grapheme \\*%\hline
		&\Grapheme{tt}		& before unvoiced or preaspirated C \\\hline %; (inconsistently intervocalically)	\\\hline
\IPA{ʰtː}	&\Grapheme{htt}	& default grapheme	\\\hline
\IPA{k}	&\Grapheme{g}		& default grapheme \\*
		&\Grapheme{k}		& word-finally; in C-cluster except after sonorant C\\\hline
\IPA{ʰk}	&\Grapheme{hk}	& default grapheme	\\*
		&\Grapheme{k}		& after sonorant C in C-center \\\hline%& \It{fuällke} ‘family’ \\\hline
\IPA{kː}	&\Grapheme{gg}	& default grapheme \\*%\hline
		&\Grapheme{kk}	& before unvoiced or preaspirated C \\\hline %; (inconsistently intervocalically)	\\\hline
\IPA{ʰkː}	&\Grapheme{hkk}	& default grapheme	\\\hline

\IPA{ʦ}	&\Grapheme{ts}	& default grapheme	\\\hline
%		&\Grapheme{dts}	& /ʦː/ (geminate) \\\hline
\IPA{ʰʦ}	&\Grapheme{hts}	& default grapheme	\\*
...
\IPA{ʦː}	&\Grapheme{dts}	& default grapheme \\\hline
%		&\Grapheme{tts}	& before unvoiced or preaspirated C \\\hline %; (inconsistently intervocalically)	\\\hline
\IPA{ʰʦː}	&\Grapheme{htts}	& default grapheme	\\\hline

\IPA{ʧ}	&\Grapheme{tj}		& default grapheme	\\\hline
%		&\Grapheme{dtj}	& /ʃː/ (geminate) \\\hline
\IPA{ʰʧ}	&\Grapheme{htj}	& default grapheme	\\*
...
\IPA{ʧː}	&\Grapheme{dtj}	& default grapheme \\\hline
%		&\Grapheme{ttj}	& second member C \\\hline %; (inconsistently intervocalically)	\\\hline
\IPA{ʰʧː}	&\Grapheme{httj}	& default grapheme	\\\hline

\IPA{f}	&\Grapheme{f}		& default grapheme	\\\hline
\IPA{f:}	&\Grapheme{ff}		& default grapheme	\\\hline
\IPA{v}	&\Grapheme{v}		& default grapheme	\\\hline
\IPA{vː}	&\Grapheme{vv}	& default grapheme	\\\hline
\IPA{s}	&\Grapheme{s}		& default grapheme	\\\hline
\IPA{sː}	&\Grapheme{ss}	& default grapheme	\\\hline
\IPA{ʃ}	&\Grapheme{sj}	& default grapheme	\\\hline
\IPA{ʃː}	&\Grapheme{ssj}	& default grapheme	\\\hline
...
\IPA{mː}	&\Grapheme{mm}	& default grapheme	\\\hline
\IPA{n}	&\Grapheme{n}		& default grapheme	\\\hline
\IPA{nː}	&\Grapheme{nn}	& default grapheme	\\\hline
\IPA{ɲ}	&\Grapheme{nj}	& default grapheme	\\\hline
\IPA{ɲː}	&\Grapheme{nnj}	& default grapheme	\\\hline
%		&\Grapheme{nnj}	& sequence of two /ɲ/ (geminate)	\\\hline
\IPA{ŋ}	&\Grapheme{ŋ}		& default grapheme	\\\hline
\IPA{ŋː}	&\Grapheme{ŋŋ}	& default grapheme	\\\hline
\IPA{r}	&\Grapheme{r}		& default grapheme	\\\hline
\IPA{rː}	&\Grapheme{rr}		& default grapheme	\\\hline
\IPA{l}	&\Grapheme{l}		& default grapheme	\\\hline
\IPA{lː}	&\Grapheme{ll}		& default grapheme	\\\hline
\IPA{j}	&\Grapheme{j}		& default grapheme	\\\hline
\IPA{jː}	&\Grapheme{jj}		& default grapheme	\\\hline
%\end{tabular}
\end{longtable}
\label{orthTableCend}
